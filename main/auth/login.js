...
let isHandlingGoogleLogin = false;
...
const unsubscribe = onAuthStateChanged(auth, (user) => {
  unsubscribe();
  if (isHandlingGoogleLogin) return;
  if (user) {
...
document.addEventListener("DOMContentLoaded", () => {
  loginCard.classList.add('form-active');
  signupCard.classList.add('form-hidden');
...
document.getElementById('loginBtn').addEventListener('click', async (event) => {
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      const redirectURL = sessionStorage.getItem('redirectAfterLogin') || '/feed.html';
      sessionStorage.removeItem('redirectAfterLogin');
      window.location.href = redirectURL;
...
document.getElementById("googleLoginBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    if (isNewUser) {
      ...
      redirectURL = savedRedirect || 'feed.html';
    }
...
document.getElementById("showPassword").addEventListener("click", async () => {
  const selectedLang = getCurrentLang();
...
document.getElementById('forgetPassword').addEventListener('click', async () => {
...
document.getElementById("signupshowPassword").addEventListener("click", async () => {
  const selectedLang = getCurrentLang();
  const t = Signup_AuthForm_translations[selectedLang] || Signup_AuthForm_translations['en'];
...
document.getElementById('switchToSignup').addEventListener('click', () => {
  switchForm(loginCard, signupCard);
});

document.getElementById('switchToLogin').addEventListener('click', () => {
  switchForm(signupCard, loginCard);
});
...
function getCurrentLang() {
  const languageSelect = document.querySelector('#languageSelectorLogin');
  return (
...
initLanguageSelector('languageSelectorLogin');
...
function setBodyFont(lang) {
  ...
  const landing = document.querySelector(".cvb-LandingStyle");
...
function applyTranslations(lang) {
  AuthForm_translatePage(lang);
  signup_AuthForm_translatePage(lang);
  initializeLoginEvents();
  translateHelpModal(lang);
  setBodyFont(lang);
}

function AuthForm_translatePage(lang) {
  ...
  // Main login fields
  loginPage.querySelector('#loginModalTitle').textContent = t["login-title"];
  loginPage.querySelector('#loginModalSubTitle').textContent = t["login-subtitle"];
  loginPage.querySelector('#loginEmail').placeholder = t["login-email"];
  loginPage.querySelector('#loginPassword').placeholder = t["login-password"];
  const showPass = loginPage.querySelector('#showPassword');
  if (showPass) showPass.textContent = t["show-password"];
  const forgetPass = loginPage.querySelector('#forgetPassword');
  if (forgetPass) forgetPass.textContent = t["forgot-password"];
  loginPage.querySelector('#loginModalTitleReset').textContent = t["login-titleReset"];
  loginPage.querySelector('#loginModalSubTitleReset').textContent = t["login-subtitleReset"];
  const sendPass = loginPage.querySelector('#sendpassword');
  if (sendPass) sendPass.textContent = t["send-password"];
  const backLogin = loginPage.querySelector('#backLogin');
  if (backLogin) backLogin.textContent = t["login-back"];
  const loginBtn = loginPage.querySelector('#loginBtn');
  if (loginBtn) loginBtn.textContent = t["login-btn"];

  // Signup
  const loginSignup = loginPage.querySelector('.login-signup');
  if (loginSignup) {
    loginSignup.childNodes[0].textContent = `${t["no-account"]} `;
    loginSignup.querySelector('#switchToSignup').textContent = t["signup-link"];
  }

  const googleBtn = loginPage.querySelector('#googleLoginBtn span');
  if (googleBtn) googleBtn.textContent = t["continue-google"];

  // ===== Overlay text =====
  const overlay = loginPage.querySelector('.login-overlay');
  if (overlay) {
    overlay.querySelector('h2').textContent = t["overlay-title"];
    overlay.querySelector('p').innerHTML = t["overlay-text"].replace(/\n/g, "<br>");
  }

  // ===== Footer links =====
  const footerLinks = loginPage.querySelector('.login-footer-links');
  if (footerLinks) {
    ...
    if (links[2]) links[2].textContent = t["footer-faq"];
  }

  // ===== Footer copyright =====
  const footerCopyright = loginPage.querySelector('.login-footer-copyright');
  if (footerCopyright) footerCopyright.textContent = t["footer-copyright"];
}

function signup_AuthForm_translatePage(lang) {
  const t = Signup_AuthForm_translations[lang] || Signup_AuthForm_translations['en'];

  const signupCard = document.getElementById('signup-card');
  if (!signupCard) return;

  // ===== Titles =====
  const title = signupCard.querySelector('#signupModalTitle');
  if (title) title.textContent = t["signup-title"];

  const subtitle = signupCard.querySelector('#signupModalSubTitle');
  if (subtitle) subtitle.textContent = t["signup-subtitle"];

  // ===== Inputs =====
  const firstName = signupCard.querySelector('#signupFirstName');
  if (firstName) firstName.placeholder = t["signup-first-name"];

  const lastName = signupCard.querySelector('#signupLastName');
  if (lastName) lastName.placeholder = t["signup-last-name"];

  const email = signupCard.querySelector('#signupEmail');
  if (email) email.placeholder = t["signup-email"];

  const birthday = signupCard.querySelector('#signupBirthday');
  if (birthday) birthday.placeholder = t["signup-birthday"];

  const password = signupCard.querySelector('#signupPassword');
  if (password) password.placeholder = t["signup-password"];

  // ===== Show password =====
  const showPass = signupCard.querySelector('#signupshowPassword');
  if (showPass) showPass.textContent = t["show-password"];

  // ===== Button =====
  const signupBtn = signupCard.querySelector('#signupBtn');
  if (signupBtn) signupBtn.textContent = t["signup-btn"];

  // ===== Already have account =====
  const paragraph = signupCard.querySelector('#signup-AlreadyAccount');
  if (paragraph) {
    // Fix text node safely
    paragraph.childNodes[0].textContent = `${t["have-account"]} `;
  }

  const loginLink = signupCard.querySelector('#switchToLogin');
  if (loginLink) loginLink.textContent = t["login-link"];
}
