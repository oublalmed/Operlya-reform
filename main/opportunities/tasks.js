...
async function loadRightBar() {
  ...
  const networkContainer = document.getElementById("right-network");
  ...
  const tStatus = cvbTaskStatusTranslations[lang] || cvbTaskStatusTranslations["en"];
  const tChat = cvbChatTranslations[lang] || cvbChatTranslations["en"];
  const tForm = UserForm_translations[lang] || UserForm_translations["en"];
  const tRight = cvbRightBarTranslations[lang] || cvbRightBarTranslations["en"];
  ...
  // ================= NETWORK =================
  async function loadLiveNetwork(user) {
    const container = document.getElementById("right-network");
    if (!container) return;

    container.innerHTML = `
      <div class="network-title">${tChat.executionMsgs}</div>
    ...
    const tasksSnap = await getDocs(
      query(
        ...
        where("status", "in", ["Open", "Assigned", "Progressing", "Completed"])
      )
    );

    for (const taskDoc of tasksSnap.docs) {

      const opportunityId = taskDoc.id;
      const task = taskDoc.data();

      const applyRef = doc(db, "opportunities", opportunityId, "applies", user.uid);
      const applySnap = await getDoc(applyRef);

      if (!applySnap.exists()) continue;

      const applyStatus = applySnap.data().status;

      if (!["accepted", "assigned", "progressing", "completed"].includes(applyStatus)) {
        continue;
      }

      const chatId = `private_${user.uid}`;
      const chatRef = doc(db, "opportunities", opportunityId, "chats", chatId);

      onSnapshot(chatRef, async (chatSnap) => {

        // ===== OWNER INFO =====
        let ownerName = tRight.owner;
        let ownerStatus = "";
        ...
        try {
          if (task.ownerId) {
            ...
            if (ownerSnap.exists()) {
              ...
              const roleKey = (ownerData.role || "").toLowerCase();
              ownerStatus = tForm[`role-${roleKey}`] || roleKey || "";
            }
          }
        } catch (err) {
        ...
        let senderName = tChat.members;
        try {
          if (msg.senderId) {
            const senderSnap = await getDoc(doc(db, "profiles", msg.senderId));
        ...
        let lastMessage = tChat.emptyTitle;
        let lastTime = "";
        ...
        try {
          ...
          if (!msgSnap.empty) {
            ...
            lastMessage = `${senderName}: ${msg.text || ""}`;
        ...
        let item = document.getElementById(`task-${opportunityId}`);

        if (!item) {
          item = document.createElement("div");
          item.className = "network-task";
          item.id = `task-${opportunityId}`;
          list.appendChild(item);
        }

        const statusLabel = tStatus[applyStatus] || applyStatus;
        const statusClass = applyStatus || "pending";

        item.innerHTML = `
          <div class="network-owner">${ownerName} - ${ownerStatus}</div>
          <div class="network-task-title">${task.title || tRight.task}</div>
          <div class="network-task-message"><span>${lastMessage}</span></div>
          <div class="network-task-footer">
            <div class="network-statusGroup">
              <div class="network-statusDot ${statusClass}"></div>
              <span class="network-statusText">${statusLabel}</span>
            </div>
            <div class="network-task-time">
              ${lastTime}
            </div>
          </div>
        `;
        ...
        item.addEventListener("click", () => {
          window.location.href = `opportunities/execution.html?opportunityId=${opportunityId}`;
        });
      });
    }
  }

  await loadLiveNetwork(user);
}

let profileCoverAnimation = null;

function renderProfileCard(data, user) {
  const lang = getCurrentLang();
  ...
  const avatar = data.avatar || "images/default_avatar.png";
  const headline = data.headline || tForm["operlya-member"] || "Operlya Member";
  const availability = data.availability || "";
  const role = (data.role || "talent").toLowerCase();
  ...
  if (profileIndustry) {
    if (data.industries && data.industries.length) {
      profileIndustry.textContent = data.industries
        .map(ind => {
          const industryLabel =
            tIndustry[ind.nameKey] || ind.name || ind.id;
      ...
      profileIndustry.textContent = tForm["placeholder-industry"] || "Not specified";
    }
  }
  ...
  if (profileVerified) {
    if (verified) {
      profileVerified.style.display = "flex";
      const span = profileVerified.querySelector("span");
      if (span) {
        span.textContent =
  ...
  if (profileRole) {
    ...
    const span = profileRole.querySelector("span");
    if (span) {
      span.textContent = roleLabels[role] || role;
    }

    profileRole.className = `profile-badge role ${role}`;
  }

  // ================= AVAILABILITY =================
  if (profileAvailability) {
    if (availability) {
      const span = profileAvailability.querySelector("span");
      if (span) {
        span.textContent =
          tForm[`availability-${availability}`] || availability;
      }

      profileAvailability.className =
        `profile-badge availability ${availability}`;

      profileAvailability.style.display = "flex";
    } else {
      profileAvailability.style.display = "none";
    }
  }

  // ================= COVER =================
  const industryId = data.industries?.[0]?.id || "default";
...
function calculateProfileCompletion(data) {
  ...
  if (data.first_name) score++;
  if (data.last_name) score++;
  if (data.profile?.avatar) score++;
  if (data.profile?.headline) score++;
  if (data.profile?.industry) score++;
  if (data.profile?.bio) score++;

  return Math.round((score / total) * 100);
}

async function loadProfileKPIs(user) {
  const profileCard = document.querySelector(".profile-card");
  if (!profileCard || !user) return;

  const tRight = cvbRightBarTranslations[getCurrentLang()] || {};

  const stats = await getUserApplyStats(user);

  let kpiBlock = profileCard.querySelector(".profile-kpis");

  if (!kpiBlock) {
    kpiBlock = document.createElement("div");
    kpiBlock.className = "profile-kpis";
    profileCard.appendChild(kpiBlock);
  }
  ...
  kpiBlock.innerHTML = `
    <div class="profile-kpi">
      <div>
        ...
        <span>${tRight.applies || "Applies"}</span>
      </div>
    </div>
    ...
    <div class="profile-kpi">
      <div>
        ...
        <span>${tRight.inProgress || "In Progress"}</span>
      </div>
    </div>
  `;
}

async function getUserApplyStats(user) {
  const q = query(
    collection(db, "opportunities"),
  ...
  for (const docSnap of snap.docs) {
    ...
    if (["accepted", "assigned", "progressing"].includes(status)) {
      counts.inProgress++;
    }
...
const complexityInput = document.getElementById("complexityValue");
const complexitySlider = document.getElementById("complexityRange");
function updateSliderBackground(value) {
  const min = complexitySlider.min;
  const max = complexitySlider.max;
  const percent = ((value - min) / (max - min)) * 100;
  complexitySlider.style.background = `linear-gradient(
    to right,
    #3b82f6 0%,
    #3b82f6 ${percent}%,
    #ddd ${percent}%,
  ...
  complexityInput.textContent = value + "%";
}
complexitySlider.addEventListener("input", () => {
  updateSliderBackground(complexitySlider.value);
});
updateSliderBackground(complexitySlider.value);

const filterBox = document.querySelector(".tasksFilter");
...
item.addEventListener("click", async () => {
  if (item.querySelector(".lb-soon")) return;
    document.querySelectorAll(".lb-item")
...
document.querySelectorAll("#locationFilter,#bytextFilter,.tf-group input,#complexityRange").forEach(el=>{
...
document.querySelectorAll(".mobile-nav-item").forEach(btn => {
  ...
  btn.addEventListener("click", async () => {
    ...
    if (filter === "network") {
      ...
      if (rightBar) rightBar.classList.add("mobile-active");
    ...
    // ===== NORMAL TASKS VIEW =====
    if (rightBar) rightBar.classList.remove("mobile-active");
...
function ensureLoadMore(container) {
  let wrapper = container.querySelector(".load-more-container");
  ...
  if (!wrapper) {
    ...
    wrapper.innerHTML = `
      <span id="loadMoreTasks" class="load-more-btn">
        Load more
      </span>
    `;
    ...
    wrapper.querySelector("#loadMoreTasks").addEventListener("click", () => {
      if (isLoadingTasks) return;
      renderNextTasks(currentFilter);
    });
  }
...
function toggleLoadMoreButton() {
  ...
  const wrapper = ensureLoadMore(footer);
...
async function renderNextTasks(filter = "all") {
  const user = auth.currentUser;
  const container = document.getElementById("tasksContainer");
  if (!container) return;
  ...
  while (currentIndex < allTasks.length && rendered < TASKS_STEP) {
    ...
    if (["all", "jobs", "tasks", "projects", "benevols"].includes(filter)) {
      if (filter === "all") show = true;
      else if (task.category === filter) show = true;
    }

    if (filter === "saved" && user) {
      const saveSnap = await getDoc(doc(db, "opportunities", task.id, "saves", user.uid));
      show = saveSnap.exists();
    }

    if (filter === "applied" && user) {
      const applySnap = await getDoc(doc(db, "opportunities", task.id, "applies", user.uid));
      show = applySnap.exists();
    }

    if (show && passesUIFilters(task)) {
      const exists = container.querySelector(`[data-task-id="${task.id}"]`);
      if (exists) continue;
      const card = task.category === "jobs"
        ? await createSmallJobCard(task)
        : await createSmallTaskCard(task);
...
function getUIFilters() {
  ...
  const text = document.getElementById("bytextFilter").value.toLowerCase();
  const complexity = parseInt(document.getElementById("complexityRange").value);

  const settings = [];

  document.querySelectorAll(".tf-group input:checked").forEach(cb=>{
    settings.push(cb.value);
  });
...
function passesUIFilters(task) {
  ...
  if (f.location) {
    ...
    const loc = task.location?.reference?.toLowerCase() || "";
  ...
  if (f.settings.length > 0) {
    ...
    if (!f.settings.includes(task.workSetting)) return false;

  }

  /* ---------- COMPLEXITY ---------- */

  const complexityScore = computeTaskComplexity(task);

  if (complexityScore > f.complexity) return false;

  return true;

}

function computeTaskComplexity(task){
  let score = 0;
  if(task.evaluation?.criteria?.qualityScore) score += 20;
  if(task.evaluation?.revision?.allowed) score += 10;
  if(task.schedule?.timeFlexibility === "Strict") score += 20;
  if(task.schedule?.estimatedDuration > 20) score += 15;
  if(task.guarantees?.latePenalty) score += 15;
  if(task.skills?.length > 3) score += 10;
  if(task.workDynamics === "Team") score += 10;
  return Math.min(score,100);
}

// ================================ Premium Tasks ==================================

async function loadPremiumTasks() {
  
  const lang = getCurrentLang();
  const t = cvbTaskFeedTranslations[lang] || cvbTaskFeedTranslations["en"];
  const f = taskForm_translations[lang] || taskForm_translations["en"];
  const tIndustry = Industry_Speciality_translations[lang] || Industry_Speciality_translations["en"];
  const tUser = UserForm_translations[lang] || UserForm_translations['en'];

  const section = document.getElementById("premiumTasks");
  if (!section) return;
  ...
  try {
    ...
    header.innerHTML = `
      <div class="premium-badge">
        ...
        ${t.premiumOpportunities || "Premium Opportunities"}
      </div>

      <div class="premium-nav">
        <button title="${t.previous || "Previous tasks"}" id="premiumPrev">
          <i class="ri-arrow-left-s-line"></i>
        </button>
        <button title="${t.next || "Next tasks"}" id="premiumNext">
    ...
    wrapper.className = "premium-slider-wrapper";

    const slider = document.createElement("div");
    slider.className = "premium-slider";
    slider.id = "premiumTasksSlider";
    ...
    for (const docSnap of snap.docs) {
      ...
      const paymentModelKey = task.paymentModel
        ? `payment-model-${task.paymentModel.toLowerCase()}`
        : "";

      const paymentModelLabel =
        (paymentModelKey && f[paymentModelKey]) ||
        task.paymentModel ||
        (f.paymentModelNotSelected || "Payment model not selected");

      const priceLabel = price.amount
        ? `${price.amount} ${price.currency || ""} · ${paymentModelLabel}`
      ...
      const typeLabel = task.type
        ? `${f[`work-nature-${task.type?.toLowerCase()}`] || task.type} ${t.in || "in"} ${categoryLabel}`
        : categoryLabel || (t.task || "Task");
      ...
      if (task.ownerId) {
        const ownerDoc = await getDoc(doc(db, "profiles", task.ownerId));

        if (ownerDoc.exists()) {
          const u = ownerDoc.data();
          ownerAvatar = u.avatar || ownerAvatar;
      ...
      card.innerHTML = `
        <div class="premium-owner">
          <img src="${ownerAvatar}">
          <div class="premium-ownerDetails">
            <span class="premium-ownerName">${ownerName}</span>
            <div class="premium-ownerTitle">${ownerRole}</div>
          </div>
        </div>
      ...
      card.onclick = () => openPremiumTask(id);

      slider.appendChild(card);
    }

    // ===== NAV EVENTS (after DOM creation) =====
    const sliderEl = document.getElementById("premiumTasksSlider");

    document.getElementById("premiumNext").onclick = () => {
      sliderEl.scrollBy({ left: 220, behavior: "smooth" });
    };

    document.getElementById("premiumPrev").onclick = () => {
      sliderEl.scrollBy({ left: -220, behavior: "smooth" });
    };
  } catch (error) {
...
async function openPremiumTask(opportunityId){

  const container = document.getElementById("tasksContainer");
  if(!container) return;

  const exists = container.querySelector(`[data-opportunity-id="${opportunityId}"]`);
  if (exists) {
    exists.scrollIntoView({ behavior: "smooth", block: "center" });
    return;
  }

  const alreadyLoaded = allTasks.find(t => t.id === opportunityId);
  if (alreadyLoaded) {
    const card = alreadyLoaded.category === "jobs"
      ? await createSmallJobCard(alreadyLoaded)
      : await createSmallTaskCard(alreadyLoaded);
    container.appendChild(card);
    return;
  }
...
function showEmptyTasks() {
  const selectedLang = getCurrentLang();
  const t = cvbTaskFeedTranslations[selectedLang] || cvbTaskFeedTranslations['en'];
...
async function createSmallTaskCard(task) {

  if (task.category === "jobs") {
    return await createSmallJobCard(task);
  }
  ...
  const user = auth.currentUser;
  const isOwner = user && user.uid === task.ownerId;

  const card = document.createElement("div");
  card.className = "pv-taskCard";
  card.dataset.opportunityId = task.id;
  ...
  // ===== STRUCTURE BUILDER =====
  card.innerHTML = buildSmallTaskCardHTML(task, ownerData, isOwner);

  // Increment views
  if (task.id) {
    const taskRef = doc(db, "opportunities", task.id);
    updateDoc(taskRef, {
  ...
  setTimeout(async () => {
    ...
    // ===== SAVE BUTTON =====
    const saveBtn = card.querySelector(".pv-saveBtn");
    const user = auth.currentUser;

    if (user && saveBtn) {
      const saveRef = doc(db, "opportunities", task.id, "saves", user.uid);
      const saveSnap = await getDoc(saveRef);

      if (saveSnap.exists()) {
        saveBtn.classList.add("saved");
        saveBtn.innerHTML = `<i class="ri-bookmark-fill"></i> ${t.saved}`;
      }
      ...
      saveBtn.onclick = async () => {
        ...
        if (snap.exists()) {
          ...
          saveBtn.innerHTML = `<i class="ri-bookmark-line"></i> ${t.save}`;
        } else {
    ...
    // ===== APPLY BUTTON =====
    const applyBtn = card.querySelector(".pv-applyBtn");
    if (applyBtn && user && !isOwner) {
      const applyRef = doc(db, "opportunities", task.id, "applies", user.uid);

      const applySnap = await getDoc(applyRef);
      if (applySnap.exists()) {
        applyBtn.classList.add("applied");
        applyBtn.innerHTML = `<i class="ri-check-line"></i> ${t.applied}`;
      }
...
function buildSmallTaskCardHTML(task, ownerData, isOwner = false) {
  ...
  /* -------------------------------------------------- */
  /* SMALL INTERNAL HELPERS (no structural change)     */
  /* -------------------------------------------------- */
  ...
  const typeLabel = task.type
    ? `${f[`work-nature-${task.type?.toLowerCase()}`] || task.type || "Task"} ${t.in || "in"} ${categoryspecialityLabel}`
    : categoryspecialityLabel || "Task";
  ...
  if (schedule.startDate || schedule.endDate) {
    if (schedule.startDate && schedule.endDate)
      ...
      periodHTML = `
        <span class="period duration">
          <i class="ri-calendar-line"></i>
          ${f['from'] || 'From'} ${formatDate(schedule.startDate, t)}
        </span>
      `;
    else
      periodHTML = `
        <span class="period duration">
          <i class="ri-calendar-line"></i>
          ${f['until'] || 'Until'} ${formatDate(schedule.endDate, t)}
        </span>
      `;
  }
  ...
  const deadlineHTML = schedule.deadline
    ? `
      <span class="deadline">
        ...
        ${f['hard-deadline-label'] || 'Deadline'}: ${formatDate(schedule.deadline, t)}
      </span>
    `
    : "";
  ...
  const actionButtonsHTML = `
    <div class="pv-actionBar">
      <button class="pv-saveBtn">
        <i class="ri-bookmark-line"></i>
        ${t.save || "Save Task"}
      </button>
      <button class="pv-applyBtn">
...
async function createSmallJobCard(task) {
  ...
  const card = document.createElement("div");
  card.className = "pv-taskCard pv-jobCard";
  card.dataset.opportunityId = task.id;
      ...
      if (ownerSnap.exists()) {
        ...
        if (data.industries?.length) {
          ownerData.ownerIndustry = data.industries
            .map(ind => {
              const industryLabel =
                tIndustry[`industry-${ind.name?.toLowerCase()}`] ||
                tIndustry[ind.nameKey] ||
                ind.name ||
                ind.id;

              const specialityLabel = ind.speciality
                ? (tIndustry[`speciality-${ind.speciality}`] || ind.speciality)
                : ind.specialityKey
                  ? tIndustry[ind.specialityKey]
                  : null;
  ...
  // ===== HTML =====
  card.innerHTML = buildSmallJobCardHTML(task, ownerData, isOwner);

  // views
  updateDoc(doc(db, "opportunities", task.id), {
    viewsCount: increment(1)
  }).catch(() => {});

  // actions
  setTimeout(() => setupJobCardActions(card, task, isOwner), 0);

  return card;
}

function buildSmallJobCardHTML(task, ownerData, isOwner = false) {
  ...
  return `
    ...
    ${!isOwner ? `
      <div class="pv-actionBar">
        <button class="pv-saveBtn">
          <i class="ri-bookmark-line"></i> ${t.save || "Save Job"}
        </button>
        <button class="pv-applyBtn">
          <i class="ri-send-plane-line"></i> ${t.apply || "Apply"}
        </button>
      </div>
    ` : ""}
  `;
...
function setupJobCardActions(card, task, isOwner) {
  ...
  const saveBtn = card.querySelector(".pv-saveBtn");
  if (saveBtn && user) {
    const ref = doc(db, "opportunities", task.id, "saves", user.uid);
    ...
    saveBtn.onclick = async () => {
      ...
      if (snap.exists()) {
        ...
        await setDoc(ref, { userId: user.uid, createdAt: serverTimestamp() });
        saveBtn.classList.add("saved");
        saveBtn.innerHTML = `<i class="ri-bookmark-fill"></i> ${t.saved}`;
      }
    };
    ...
    card.addEventListener("click", (e) => {
      if (e.target.closest(".pv-actionBar")) return;
      window.open(task.offerLink, "_blank", "noopener,noreferrer");
    });
    const applyBtn = card.querySelector(".pv-applyBtn");
    if (applyBtn) {
      applyBtn.innerHTML = `<i class="ri-external-link-line"></i> ${t.viewJob || "Open"}`;
      applyBtn.onclick = (e) => {
        e.stopPropagation();
        window.open(task.offerLink, "_blank", "noopener,noreferrer");
      };
  ...
  const applyBtn = card.querySelector(".pv-applyBtn");
  if (applyBtn && user && !isOwner) {
    const ref = doc(db, "opportunities", task.id, "applies", user.uid);
...
let allExternalTasks = [];
let externalIndex = 0;
let isLoadingExternal = false;

const EXTERNAL_STEP = 3;

async function loadExternalOpportunities() {

  const lang = getCurrentLang();

  const t =
    externalTasksTranslations[lang] ||
    externalTasksTranslations.en;

  const container =
    document.getElementById("externalTasksContainer");
  ...
  try {
    ...
    allExternalTasks = snap.docs.map(doc => {
    ...
    allExternalTasks.sort(
      (a, b) => b.createdAtMs - a.createdAtMs
    );

    // ================= EMPTY =================

    if (!allExternalTasks.length) {
...
function renderNextExternal() {
  const container = document.getElementById("externalTasksContainer");
  if (!container) return;

  let rendered = 0;

  while (externalIndex < allExternalTasks.length && rendered < EXTERNAL_STEP) {
    const task = allExternalTasks[externalIndex];
    externalIndex++;

    const card = createExternalOpportunityCard(task);
    container.appendChild(card);

    rendered++;
  }

  toggleExternalLoadMore();
}

function ensureExternalLoadMore() {
  const lang = getCurrentLang();
  const t = externalTasksTranslations[lang] || externalTasksTranslations.en;

  const section = document.querySelector(".externalTasksList");
  if (!section) return null;

  let wrapper = section.querySelector(".load-more-container");
  ...
  if (!wrapper) {
    ...
    wrapper.innerHTML = `
      <span id="loadMoreExternalBtn" class="load-more-btn">
        ${t.loadMore}
      </span>
    `;
    ...
    wrapper
      .querySelector("#loadMoreExternalBtn")
...
function toggleExternalLoadMore() {
  const wrapper = ensureExternalLoadMore();
  if (!wrapper) return;
  if (externalIndex >= allExternalTasks.length) {
    wrapper.style.display = "none";
  } else {
    wrapper.style.display = "flex";
  }
...
function translateTasksFeedHeader() {
  const lang = getCurrentLang();
  const t = cvbTaskFeedTranslations[lang] || cvbTaskFeedTranslations.en;

  document.querySelectorAll(".tasksFeed-header").forEach(header => {
    const title = header.querySelector("h3");
    const subtitle = header.querySelector("span");
    const badge = header.querySelector(".external-feed-badge");

      if (title) {
        title.innerHTML = `<i class="ri-global-line"></i> ${t.externalOpportunities}`;
      }

      if (subtitle) {
        subtitle.textContent = t.externalSubtitle;
      }

      if (badge) {
        badge.innerHTML = `<span class="live-dot"></span> ${t.liveSources}`;
      }

  });

  loadExternalOpportunities();
}

function translateTasksEmpty() {
  const selectedLang = getCurrentLang();
  const t = cvbTaskFeedTranslations[selectedLang] || cvbTaskFeedTranslations['en'];

  const emptyContainer = document.querySelector(".tasks-empty");

  if (!emptyContainer) return;

  const titleEl = emptyContainer.querySelector("div");
  const subtitleEl = emptyContainer.querySelector("span");

  if (titleEl) {
    const icon = titleEl.querySelector("i")?.outerHTML || "";
    titleEl.innerHTML = `${icon} ${t.emptyTitle}`;
  }

  if (subtitleEl) {
    subtitleEl.textContent = t.emptySubtitle;
  }
}

function translateLeftBar() {
  const selectedLang = getCurrentLang();
  const t = cvbLeftBarTranslations[selectedLang] || cvbLeftBarTranslations['en'];

  /* ===== MAIN MENU ===== */
  const items = document.querySelectorAll(".leftBar .lb-item");

  if (items[0]) items[0].querySelector("span").textContent = t.exploreAll;
  if (items[1]) items[1].querySelector("span").textContent = t.tasks;
  if (items[2]) items[2].querySelector("span").textContent = t.jobs;
  if (items[3]) items[3].querySelector("span").textContent = t.projects;
  if (items[4]) items[4].querySelector("span").textContent = t.benevols;

  /* ===== SOON LABELS ===== */
  document.querySelectorAll(".lb-soon").forEach(el => {
    el.textContent = t.soon;
  });

  /* ===== SECOND SECTION ===== */
  if (items[5]) items[5].querySelector("span").textContent = t.trending;
  if (items[6]) items[6].querySelector("span").textContent = t.recent;

  /* ===== SAVED / APPLIED ===== */
  if (items[7]) items[7].querySelector("span").textContent = t.savedTasks;
  if (items[8]) items[8].querySelector("span").textContent = t.appliedTasks;

  /* ===== FILTER CARD ===== */
  const title = document.querySelector(".tf-title");
  if (title) {
    title.childNodes[title.childNodes.length - 1].textContent =
      " " + t.findOpportunities;
  }

  const locationInput = document.getElementById("locationFilter");
  if (locationInput) locationInput.placeholder = t.searchLocation;

  const textInput = document.getElementById("bytextFilter");
  if (textInput) textInput.placeholder = t.searchMarket;

  /* ===== CHECKBOXES ===== */
  const checkboxes = document.querySelectorAll(".tf-checkbox span");

  if (checkboxes[0]) checkboxes[0].textContent = t.remote;
  if (checkboxes[1]) checkboxes[1].textContent = t.onsite;
  if (checkboxes[2]) checkboxes[2].textContent = t.hybrid;
  if (checkboxes[3]) checkboxes[3].textContent = t.independent;

  /* ===== COMPLEXITY ===== */
  const complexityLabel = document.querySelector(".tf-complexity-header span:first-child");
  if (complexityLabel) complexityLabel.textContent = t.complexityLevel;
}
...
const cvbTaskFeedTranslations = {
  en: {
    emptyTitle: "New opportunities will be here soon",
    emptySubtitle: "Check back later or adjust your filters",
    premiumOpportunities: "Premium Opportunities",
    previous: "Previous tasks",
    next: "Next tasks",

    externalOpportunities: "External Opportunities",
    externalSubtitle:
      "Curated jobs and missions imported from trusted platforms",
    liveSources: "Live Sources"
  },

  fr: {
    emptyTitle: "De nouvelles opportunités arriveront bientôt",
    emptySubtitle: "Revenez plus tard ou ajustez vos filtres",
    premiumOpportunities: "Opportunités premium",
    previous: "Tâches précédentes",
    next: "Tâches suivantes",

    externalOpportunities: "Opportunités externes",
    externalSubtitle:
      "Emplois et missions sélectionnés depuis des plateformes fiables",
    liveSources: "Sources en direct"
  },

  ar: {
    emptyTitle: "ستتوفر فرص جديدة قريباً",
    emptySubtitle: "تحقق لاحقاً أو قم بتعديل الفلاتر",
    premiumOpportunities: "الفرص المميزة",
    previous: "المهام السابقة",
    next: "المهام التالية",

    externalOpportunities: "الفرص الخارجية",
    externalSubtitle:
      "وظائف ومهام مختارة من منصات موثوقة",
    liveSources: "مصادر مباشرة"
  },

  es: {
    emptyTitle: "Nuevas oportunidades estarán disponibles pronto",
    emptySubtitle: "Vuelve más tarde o ajusta tus filtros",
    premiumOpportunities: "Oportunidades premium",
    previous: "Tareas anteriores",
    next: "Tareas siguientes",

    externalOpportunities: "Oportunidades externas",
    externalSubtitle:
      "Trabajos y misiones seleccionados de plataformas confiables",
    liveSources: "Fuentes en vivo"
  },

  de: {
    emptyTitle: "Neue Möglichkeiten werden bald verfügbar sein",
    emptySubtitle: "Schau später wieder vorbei oder passe deine Filter an",
    premiumOpportunities: "Premium-Chancen",
    previous: "Vorherige Aufgaben",
    next: "Nächste Aufgaben",

    externalOpportunities: "Externe Möglichkeiten",
    externalSubtitle:
      "Kurierte Jobs und Missionen aus vertrauenswürdigen Plattformen",
    liveSources: "Live-Quellen"
  },

  pt: {
    emptyTitle: "Novas oportunidades estarão disponíveis em breve",
    ...
    externalOpportunities: "Oportunidades externas",
    externalSubtitle:
      "Empregos e missões selecionados de plataformas confiáveis",
    liveSources: "Fontes ao vivo"
  },

  zh: {
    emptyTitle: "新的机会即将到来",
    emptySubtitle: "请稍后再来或调整筛选条件",
    premiumOpportunities: "高级机会",
    previous: "上一任务",
    next: "下一任务",

    externalOpportunities: "外部机会",
    externalSubtitle:
      "从可信平台导入的精选工作与任务",
    liveSources: "实时来源"
  },
  ...
  ja: {
    emptyTitle: "新しい機会はまもなく表示されます",
    emptySubtitle: "後でもう一度確認するか、フィルターを調整してください",
    premiumOpportunities: "プレミアム機会",
    previous: "前のタスク",
    next: "次のタスク",

    externalOpportunities: "外部機会",
    externalSubtitle:
      "信頼できるプラットフォームから取得した厳選ジョブとミッション",
    liveSources: "ライブソース"
  },

  ru: {
    emptyTitle: "Новые возможности скоро появятся",
    emptySubtitle: "Проверьте позже или измените фильтры",
    premiumOpportunities: "Премиум возможности",
    previous: "Предыдущие задачи",
    next: "Следующие задачи",

    externalOpportunities: "Внешние возможности",
    externalSubtitle:
      "Подобранные вакансии и миссии с надежных платформ",
...
const cvbTaskInfoTranslations = {
  ...
  de: {
    ...
    viewJob: "Aufgabe ansehen",
    closeJob: "Aufgabe schließen",
...
const cvbChatTranslations = {
  en: {
    ...
    you: "You",
    executionMsgs: "Execution messages",
  ...
  fr: {
    ...
    you: "Vous",
    executionMsgs: "Messages d'exécution",
  ...
  es: {
    ...
    you: "Tú",
    executionMsgs: "Mensajes de ejecución",
  ...
  de: {
    ...
    you: "Du",
    executionMsgs: "Ausführungsnachrichten",
  ...
  pt: {
    ...
    you: "Você",
    executionMsgs: "Mensagens de execução",
  ...
  ru: {
    ...
    you: "Вы",
    executionMsgs: "Сообщения выполнения",
    
    months: ["Январь", "Февраль", "Март", "Апрель", "Май", "Июнь", "Июль", "Август", "Сентябрь", "Октябрь", "Ноябрь", "Декабрь"]
  }
};

const cvbLeftBarTranslations = {
  en: {
    ...
    appliedTasks: "Applied Tasks",

    findOpportunities: "Find Opportunities",
    searchLocation: "Search location...",
    searchMarket: "Search by speciality or market...",
  ...
  fr: {
    ...
    appliedTasks: "Tâches postulées",

    findOpportunities: "Trouver des opportunités",
    searchLocation: "Rechercher un lieu...",
    searchMarket: "Rechercher par spécialité ou marché...",
  ...
  ar: {
    ...
    appliedTasks: "المهام المقدَّم لها",

    findOpportunities: "ابحث عن الفرص",
    searchLocation: "ابحث عن موقع...",
    searchMarket: "ابحث حسب التخصص أو السوق...",
  ...
  es: {
    ...
    appliedTasks: "Tareas aplicadas",

    findOpportunities: "Encontrar oportunidades",
    searchLocation: "Buscar ubicación...",
    searchMarket: "Buscar por especialidad o mercado...",
  ...
  de: {
    ...
    appliedTasks: "Beworbene Aufgaben",

    findOpportunities: "Opportunitäten finden",
    searchLocation: "Standort suchen...",
    searchMarket: "Nach Fachgebiet oder Markt suchen...",
  ...
  pt: {
    ...
    appliedTasks: "Tarefas candidatadas",

    findOpportunities: "Encontrar oportunidades",
    searchLocation: "Pesquisar localização...",
    searchMarket: "Pesquisar por especialidade ou mercado...",
    ...
    complexityLevel: "Nível de complexidade"
  },
  ...
  ja: {
    ...
    savedTasks: "保存したタスク",
    appliedTasks: "応募したタスク",

    findOpportunities: "機会を見つける",
    searchLocation: "場所を検索...",
    searchMarket: "専門分野または市場で検索...",
  ...
  ru: {
    ...
    appliedTasks: "Отклики на задачи",

    findOpportunities: "Найти возможности",
    searchLocation: "Поиск местоположения...",
    searchMarket: "Поиск по специальности или рынку...",
