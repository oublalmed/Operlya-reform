...
onAuthStateChanged(auth, (user) => {
  if (user) {
    ...
    loadWorkManagerDashboard(user);
    loadUserWorkTable(user);
  } else {
...
function renderEmptyTasksView(message = "Select a task", type = "default") {
  const selectedLang = getCurrentLang();
  const t = workManagerTranslations[selectedLang] || workManagerTranslations['en'];

  const container = document.getElementById("tm-tasksView");
  if (!container) return;

  const emptyState = t.emptyState || {
    defaultTitle: "Select a task",
    defaultDescription: "Choose a task from the list to view its full details, manage status, and review appliers.",
    noTasksTitle: "No tasks found",
    noTasksDescription: "You haven't created any tasks yet. Click the 'Create Task' button to get started.",
    noFilterResultsTitle: "No matching tasks",
    noFilterResultsDescription: "Try adjusting your search or filter to find what you're looking for."
  };

  let title = emptyState.defaultTitle;
  let description = emptyState.defaultDescription;
  let emptyStateType = "default";

  if (type === "noTasks") {
    title = emptyState.noTasksTitle;
    description = emptyState.noTasksDescription;
    emptyStateType = "noTasks";
  } else if (type === "noFilterResults") {
    title = emptyState.noFilterResultsTitle;
    description = emptyState.noFilterResultsDescription;
    emptyStateType = "noFilterResults";
  } else if (message && message !== "Select a task") {
    title = message;
    description = emptyState.defaultDescription;
    emptyStateType = "custom";
  }

  const appliersNotification = document.getElementById("tm-appliersNotification");
  const appliersPanel = document.getElementById("tm-appliersPanel");

  if (appliersNotification) appliersNotification.innerHTML = "";
  if (appliersPanel) appliersPanel.innerHTML = "";

  container.innerHTML = `
    <div class="tm-emptyState" data-empty-type="${emptyStateType}" data-custom-message="${title}">
      <div class="tm-emptyIcon">
...
async function loadWorkManagerDashboard(user) {
  ...
  snap.forEach(docSnap => {
    ...
    const opportunityId = docSnap.id;

    totalTasks++;
    totalViews += task.viewsCount || 0;

    if (task.status === "Open") open++;
    if (task.status === "Progressing") progress++;
    if (task.status === "Completed") completed++;

    const appliesRef = collection(db, "opportunities", opportunityId, "applies");
    applyPromises.push(getDocs(appliesRef));
  });

  const appliesSnaps = await Promise.all(applyPromises);

  appliesSnaps.forEach(appSnap => {
    totalApplies += appSnap.size;
  });

  const conversion = totalViews
    ? ((totalApplies / totalViews) * 100).toFixed(1)
    : 0;

  document.getElementById("tm-totalTasks").innerText = totalTasks;
  document.getElementById("tm-totalViews").innerText = totalViews;
  document.getElementById("tm-totalApplies").innerText = totalApplies;
  document.getElementById("tm-conversionRate").innerText = conversion + "%";
}

document.addEventListener("DOMContentLoaded", () => {
  renderEmptyTasksView();
});

// ============================= WORKSPACE NAV FORMAT ===============================

async function loadUserWorkTable(user) {
  const q = query(
    collection(db, "opportunities"),
    where("ownerId", "==", user.uid)
  );
  ...
  renderWorkDisplay(tasks);
  renderBoard(tasks);
  setupTableFilters(tasks);
}

function renderWorkDisplay(tasks) {

  const selectedLang = getCurrentLang();
  const t = cvbTaskInfoTranslations[selectedLang] || cvbTaskInfoTranslations['en'];
  const te = cvbTaskEmptyTranslations[selectedLang] || cvbTaskEmptyTranslations['en'];
  const ts = cvbTaskStatusTranslations[selectedLang] || cvbTaskStatusTranslations['en'];

  const container = document.getElementById("tasksDisplay");

  if (!tasks.length) {
    container.innerHTML = `
      <div class="tm-tasksListEmpty">
        <h3>${te.noTasksTitle}</h3>
        <p>${te.noTasksDesc}</p>
        <div>
          <a href="${te.postOpportunityHref}" class="tm-emptyBtn">${te.postOpportunityLink}</a>
          <a href="${te.learnMoreHref}" class="tm-emptyBtn">${te.learnMoreLink}</a>
        </div>
      </div>
    `;
    return;
  ...
  container.innerHTML = tasks.map(task => {
    ...
    return `
      <div class="tm-taskRow" data-id="${task.id}">
        <div class="tm-taskRow-left">
          <div class="tm-taskRow-title">
            ${task.title || "Untitled task"}
          </div>
          <div class="tm-taskRow-meta">
            <span class="tm-categoryTag ${task.category || "task"}">
              ${formatCategory(task.category)}
            </span>

            <span class="tm-statusDot ${status}"></span>
            <span class="tm-taskRow-status">
              ${ts[status] || status}
            </span>
          </div>
        </div>
        <div class="tm-taskRow-right">
  ...
  document.querySelectorAll(".tm-taskRow").forEach(row => {
    row.addEventListener("click", async () => {
      document.querySelectorAll(".tm-taskRow")
      ...
      displayJobsInView(opportunityId);
      loadWorkAppliers(opportunityId, taskData);
    });
...
function formatCategory(category) {
  ...
  const t = cvbTaskInfoTranslations[lang] || cvbTaskInfoTranslations['en'];
  ...
  const icons = {
    ...
    contribution: "ri-heart-3-line"
  };

  const label =
    t.categories?.[key] ||
    t.categories?.[key?.replace("s", "")] ||
    t.categories?.work ||
    "Work";

  const icon = icons[key] || "ri-apps-line";
...
function renderBoard(tasks) {
  ...
  WORK_PIPELINE.forEach(stage => {
    const stageTasks = tasks.filter(t => t.status === stage.key);
    const column = document.createElement("div");
    column.className = `tm-column ${stage.key}`;
    column.innerHTML = `
      <h4>
        ${stage.label}
        <span class="tm-columnCount">${stageTasks.length}</span>
      </h4>
      <div class="tm-columnBody">
        ${stageTasks.map(task => `
          <div class="tm-taskCardMini" data-id="${task.id}">
            <div class="tm-taskTitleMini">${task.title || "Untitled"}</div>
            <div class="tm-taskMetaMini">
              ${formatDate(task.createdAt, t)}
            </div>
          </div>
        `).join("")}
...
function setupTableFilters(allTasks) {
  ...
  const periodSelect = document.getElementById("tm-periodFilter");
  const startInput = document.getElementById("tm-startDate");
  const endInput = document.getElementById("tm-endDate");
  const searchInput = document.getElementById("tm-searchInput");
  ...
  function applyFilters() {
    ...
    // ===== PERIOD FILTER =====
    if (periodValue !== "all" && periodValue !== "custom") {
    ...
    renderWorkDisplay(filtered);
  }

  periodSelect.onchange = applyFilters;
  startInput.oninput = applyFilters;
  endInput.oninput = applyFilters;
  searchInput.oninput = applyFilters;
}

document.addEventListener("DOMContentLoaded", () => {

  const toggle = document.getElementById("tm-advancedToggle");
  const panel = document.getElementById("tm-advancedFilters");
  ...
  const viewButtons = document.querySelectorAll(".tm-viewBtn");
  const tableCard = document.getElementById("tm-tasksWrapper");
  const boardWrapper = document.getElementById("tm-boardWrapper");
  const ganttWrapper = document.getElementById("tm-ganttWrapper");
  ...
  function switchView(view) {
    ...
    const activeBtn = document.querySelector(`.tm-viewBtn[data-view="${view}"]`);
    if (activeBtn) activeBtn.classList.add("active");

    if (view === "table") {
      tableCard.style.display = "grid";
    }
...
// =========================== WORKSPACE SELECTED WORK ===============================

async function displayJobsInView(opportunityId) {
  ...
  const t = cvbTaskInfoTranslations[selectedLang] || cvbTaskInfoTranslations['en'];
  const ts = cvbTaskStatusTranslations[selectedLang] || cvbTaskStatusTranslations['en'];
  const tIndustry = Industry_Speciality_translations[selectedLang] || Industry_Speciality_translations["en"];
  ...
  try {
    ...
    if (!taskSnap.exists()) {
      container.innerHTML = `<p>${t.taskNotFound || "Task not found."}</p>`;
      return;
    }
    ...
    if (task.ownerId) {
      ...
      if (ownerSnap.exists()) {
        ...
        ownerData.ownerName = `${data.first_name || ""} ${data.last_name || ""}`.trim() || "Anonymous";
        ownerData.ownerAvatar = data.avatar || ownerData.ownerAvatar;
        ownerData.ownerVerified = data.verified === true;
        const roleKey = data.role
          ? `role-${data.role.toLowerCase().replace(/\s+/g, "_")}`
          : "";
        ownerData.ownerRole = roleKey ? (tUser[roleKey] || data.role) : "";

        if (data.industries && data.industries.length) {
          ownerData.ownerIndustry = data.industries
            .map(ind => {
              const industryLabel = tIndustry[ind.nameKey] || ind.name || ind.id;
              const specialityLabel = ind.specialityKey ? tIndustry[ind.specialityKey] : null;
              return specialityLabel ? `${industryLabel} • ${specialityLabel}` : industryLabel;
            })
            .join(" • ");
        }
      }
    ...
    card.className = "pv-taskCard active";

    const status = task.status || "Draft";
    const statusBadge = `<span class="pv-statusBadge ${status}">${ts[status] || status}</span>`;
    const showActions = task.status === "Draft" || task.status === "Pending";

    card.innerHTML = `
      <div class="pv-cardActions">
        ${statusBadge}
        ${showActions ? `
          <button class="pv-actionBtn edit" data-id="${task.id}">
            <i class="ri-edit-line"></i>
          </button>
          <button class="pv-actionBtn delete" data-id="${task.id}">
            <i class="ri-delete-bin-6-line"></i>
          </button>
        ` : ""}
        <button class="pv-actionBtn duplicate" data-id="${task.id}">
          <i class="ri-file-copy-line"></i>
        </button>
      </div>

      ${task.category === "jobs"
        ? buildSmallJobCardHTML(task, ownerData, true)
        : buildSmallTaskCardHTML(task, ownerData)
      }
    `;
    ...
    if (showActions) {
      card.querySelector(".pv-actionBtn.edit")?.addEventListener("click", e => {
        e.stopPropagation();
        editOpportunitie(task);
      });
      card.querySelector(".pv-actionBtn.delete")?.addEventListener("click", e => {
        e.stopPropagation();
        deleteOpportunitie(task.id);
      });
    }

    card.querySelector(".pv-actionBtn.duplicate")?.addEventListener("click", e => {
      e.stopPropagation();
      duplicateOpportunitie(task);
    });
...
function buildSmallTaskCardHTML(task, ownerData) {
  ...
  const specialityLabel = task.speciality
    ...
    : "";
  const categoryspecialityLabel = specialityLabel
    ? `${industryLabel} • ${specialityLabel}`
    : industryLabel;

  const typeKey = (task.category || task.type || "task").toLowerCase();

  const translatedType =
    t.categories?.[typeKey] ||
    t.categories?.[`${typeKey}s`] || // fallback plural
    typeKey;

  const typeLabel = categoryspecialityLabel
    ? `${translatedType} ${t.in || "in"} ${categoryspecialityLabel}`
    : translatedType;

  const skillsHTML = task.skills?.length
    ? `<div style="display: block;">
        ${task.skills.map(s => renderSavedSkillCircle(s)).join("")}
      </div>`
    : "";

  const languagesHTML = task.languages?.length
    ? `<div style="display: block;">
        ${task.languages.map(l => renderSavedSkillCircle(l)).join("")}
      </div>`
    : "";
  ...
  if (task.evaluation?.method) {
    ...
    const rules = joinSafe([
      ...
      criteria.qualityScore && criteria.minScore &&
        iconTag("ri-star-line", ` ${f['criteria-min-score'] || 'Min quality'}: ${criteria.minScore}%`, "pv-eval-rule"),
      criteria.clientApproval &&
        iconTag("ri-user-voice-line", ` ${f['criteria-client'] || 'Client approval required'}`, "pv-eval-rule"),
      revision.allowed &&
      ...
      <div class="pv-evaluation">
        <div class="pv-eval-tags">
          <span class="pv-eval-tag">
            ${f['validation-logic-label'] || 'Validation per'} ${f[`validation-${method}`.toLowerCase()] || method}
          </span>
        </div>
  ...
  const compensationHTML = `
    <div class="pv-comp-tags">
      <span class="pv-comp-tag price">
        ...
        ${price.amount ? `${price.amount} ${price.currency || ""}` : (f.priceNotSet || "Price not set")}
  ...
  const viewsCount = task.viewsCount || 0;
  const appliesCount = task.appliesCount || 0;
  const conversionRate = viewsCount
    ? ((appliesCount / viewsCount) * 100).toFixed(1)
    : 0;
  let conversionClass = "";
  if (conversionRate >= 20) conversionClass = "high";
  else if (conversionRate >= 10) conversionClass = "medium";
  else conversionClass = "low";

  const liveMetaHTML = `
    <div class="pv-liveMeta">
      <span>
      ...
      <span>
        ...
        ${appliesCount} ${t.applies}
      </span>
      <span class="pv-conversion ${conversionClass}">
        <i class="ri-line-chart-line"></i>
        ${conversionRate}% ${t.conversion}
      </span>
      <span>
        <span class="live-dot"></span>
        ${t.posted} ${formatTimeAgo(task.createdAt, t)}
      </span>
    </div>
  `;
    ...
    <div class="pv-owner">
      ...
      <div class="pv-owner-details">
        <div class="pv-owner-top">
          ...
          ${ownerVerified ? `<span class="pv-owner-badge">${t.verified || "Verified"}</span>` : ""}
        </div>
...
function buildSmallJobCardHTML(task, ownerData) {
...
function renderSavedSkillCircle(skill) {
  const maxScore = 5;
...
function formatTimeAgo(timestamp, t) {
  ...
  for (let key in intervalValues) {
    const interval = Math.floor(seconds / intervalValues[key]);
    if (interval >= 1) {
      if (interval === 1) {
        return `${interval} ${intervals[key]}`;
      } else {
        const pluralKey = key + "s";
        return `${interval} ${intervals[pluralKey] || intervals[key] + "s"}`;
      }
    }
  }
...
function editOpportunitie(task) {
  if (task.status === "Draft" || task.status === "Pending") {
    window.location.href = `create.html?edit=${task.id}`;
  }
}

async function duplicateOpportunitie(task) {
  const selectedLang = getCurrentLang();
  const t = cvbActionsTranslations[selectedLang] || cvbActionsTranslations['en'];
  ...
  try {
    await addDoc(collection(db, "opportunities"), newTask);
    showNotification("success", t.duplicateSuccessTitle, t.duplicateSuccessMsg, true);
    loadUserWorkTable(user);
  } catch (err) {
    console.error(err);
    showNotification("error", t.duplicateErrorTitle, t.duplicateErrorMsg, true);
  }
}

async function deleteOpportunitie(opportunityId) {
  const selectedLang = getCurrentLang();
  const t = cvbActionsTranslations[selectedLang] || cvbActionsTranslations['en'];
  ...
  showNotification({
    ...
    onConfirm: async () => {
      try {
        ...
        showNotification("success", t.deleteSuccessTitle, t.deleteSuccessMsg, true);
        if (user) {
          await loadUserWorkTable(user);
        }
        renderEmptyTasksView();
      } catch (err) {
        console.error("Delete error:", err);
        showNotification("error", t.deleteErrorTitle, t.deleteErrorMsg, true);
      }
    }
  });
}
...
async function loadWorkAppliers(opportunityId, taskData) {

  const selectedLang = getCurrentLang();
  const tA = cvbAppliersTranslations[selectedLang] || cvbAppliersTranslations['en'];
  const tAct = cvbAppliersActTranslations[selectedLang] || cvbAppliersActTranslations['en'];

  const panel = document.getElementById("tm-appliersPanel");
  const appliesSnap = await getDocs(collection(db, "opportunities", opportunityId, "applies"));

  const appliers = [];

  for (const applyDoc of appliesSnap.docs) {

    const applyData = applyDoc.data();
    const userId = applyData.userId;
    const applyStatus = applyData.status || "pending";
    const applierStatus = applyData.applierStatus || null;

    const userSnap = await getDoc(doc(db, "profiles", userId));
    if (!userSnap.exists()) continue;

    const userData = userSnap.data();
    const rawScore = calculateRawScore(userData.cv || {}, taskData);
    appliers.push({ userData, rawScore, userId, applyStatus, applierStatus });
  }

  const maxScore = Math.max(...appliers.map(a => a.rawScore), 1);
  
  panel.innerHTML = `
    <div class="tm-appliersPanel">
      <h3 class="tm-appliersHeader">
        ${tA.taskAppliers} (${appliers.length})
        <button class="tm-toggleAppliers">
          <i class="ri-arrow-down-s-line"></i>
        </button>
      </h3>
      <div class="tm-appliersContent"></div>
    </div>
  `;

  const content = panel.querySelector(".tm-appliersContent");

  const acceptedAppliers = appliers.filter(
    a => ["accepted", "assigned", "progressing", "completed"].includes(a.applyStatus)
  );
  const list = document.createElement("div");
  list.className = "tm-appliersList";
  content.appendChild(list);
  if (acceptedAppliers.length) {
    list.classList.add("collapsed");
  }

  appliers.forEach(({ userData, rawScore, userId, applyStatus }) => {

    const mini = document.createElement("div");
    mini.className = "tm-applierMini";

    const avatarURL =
      userData.cv?.personal?.picture || "/images/default_avatar.png";
    
    mini.innerHTML = `
      <div class="tm-applyStatus ${applyStatus}">
        ${tA[applyStatus] || applyStatus}
      </div>
      <img src="${avatarURL}" />
      <div class="tm-applierMiniContent">
        <div class="tm-applierMiniName">
          ${userData.cv?.personal?.fullName || tA.noName}
        </div>
        <div class="tm-applierMiniSpeciality">
          ${userData.cv?.personal?.speciality || ""}
        </div>
        <div class="tm-applierMiniSummary">
          ${userData.cv?.summary?.summary || ""}
        </div>
        <div class="tm-applierProgress">
          <div class="tm-progressBar" style="width:0%;"></div>
        </div>
      </div>
    `;

    const scorePercent = Math.round((rawScore / maxScore) * 100);
    mini.querySelector(".tm-progressBar").style.width = scorePercent + "%";

    list.appendChild(mini);

    mini.addEventListener("click", () => {

      const old = content.querySelector(".tm-fullCV");
      if (old) old.remove();

      const full = document.createElement("div");
      full.className = "tm-fullCV";
      full.innerHTML = buildFullCVHTML(
        userData,
        taskData,
        opportunityId,
        userId,
      ...
      const wrapper = content.querySelector(".tm-collaborationWrapper");
      if (wrapper) {
        content.insertBefore(full, wrapper);
      } else {
        content.appendChild(full);
      }
      attachFullCVActions(full, tAct, taskData);

      const closeBtn = full.querySelector(".tm-closeFullCV");
  ...
  const toggleBtn = panel.querySelector(".tm-toggleAppliers");
  ...
  if (acceptedAppliers.length) {
    ...
    acceptedSection.className = "tm-acceptedSection";

    acceptedSection.innerHTML = `
      <div class="tm-acceptedHeader">
        <div class="tm-acceptedTitle">
          ${tA.acceptedList} (${acceptedAppliers.length})
        </div>
        <button class="tm-openGroupChat">
          <i class="ri-megaphone-line"></i> ${tA.announcements}
        </button>
      </div>
      <div class="tm-acceptedList"></div>
    `;

    const collaborationWrapper = document.createElement("div");
    collaborationWrapper.className = "tm-collaborationWrapper";

    collaborationWrapper.appendChild(acceptedSection);

    const chatContainer = document.createElement("div");
    chatContainer.className = "tm-chatContainer";

    collaborationWrapper.appendChild(chatContainer);

    content.appendChild(collaborationWrapper);
    setTimeout(() => {
      openGroupChat(opportunityId, acceptedAppliers.length);
    }, 100);

    const acceptedList = acceptedSection.querySelector(".tm-acceptedList");

    acceptedAppliers.forEach(({ userData, userId, applyStatus, applierStatus }) => {

      const avatar = userData.cv?.personal?.picture || "/images/default_avatar.png";
      const card = document.createElement("div");
      card.className = "tm-acceptedCard";

      let statusStarHTML = "";
      if (applyStatus === "assigned")
        statusStarHTML = `<div class="tm-statusStar assigned"><i class="ri-user-follow-line"></i><span>${tA.assigned}</span></div>`;

      if (applyStatus === "progressing")
        statusStarHTML = `<div class="tm-statusStar progressing"><i class="ri-loader-4-line"></i><span>${tA.progressing}</span></div>`;

      if (applyStatus === "completed")
        statusStarHTML = `<div class="tm-statusStar completed"><i class="ri-checkbox-circle-line"></i><span>${tA.completed}</span></div>`;

      let notifHTML = "";
      if (["assigned", "progressing"].includes(applyStatus) && applierStatus === "progressing") {
        notifHTML = `<div class="tm-applierNotif progressing">
          <span><i class="ri-loader-2-line"></i> ${tA.workStarted}</span>
          <button class="tm-confirmProgress">${tA.confirm}</button>
        </div>`;
      }
      if (["assigned", "progressing"].includes(applyStatus) && applierStatus === "completed") {
        notifHTML = `<div class="tm-applierNotif completed">
          <span><i class="ri-checkbox-circle-line"></i> ${tA.workCompleted}</span>
          <button class="tm-confirmComplete">${tA.confirm}</button>
        </div>`;
      }

      card.innerHTML = `
        ${statusStarHTML}
        <div class="tm-acceptedInfo">
          <img src="${avatar}">
          <div class="tm-acceptedProfil">
            <div class="tm-acceptedName">${userData.cv?.personal?.fullName || tA.noName}</div>
            <div class="tm-acceptedSpec">${userData.cv?.personal?.speciality || ""}</div>
          </div>
        </div>
        ${notifHTML}
      `;
      ...
      card.addEventListener("click", () => {
        document.querySelectorAll(".tm-acceptedCard").forEach(c => c.classList.remove("active"));
        card.classList.add("active");
        openPrivateChat(opportunityId, userId);
      });

      const confirmProgressBtn = card.querySelector(".tm-confirmProgress");
      if (confirmProgressBtn) {
        confirmProgressBtn.addEventListener("click", async (e) => {
          e.stopPropagation();
          try {
            await updateDoc(doc(db, "opportunities", opportunityId, "applies", userId), { status: "progressing" });
            showNotification("success", tA.taskProgressing);
          } catch (err) {
            console.error(err);
            showNotification("error", tA.errorProgress);
          }
        });
      }

      const confirmCompleteBtn = card.querySelector(".tm-confirmComplete");
      if (confirmCompleteBtn) {
        confirmCompleteBtn.addEventListener("click", async (e) => {
          e.stopPropagation();
          try {
            await updateDoc(doc(db, "opportunities", opportunityId, "applies", userId), { status: "completed" });
            showNotification("success", tA.taskCompleted);
          } catch (err) {
            console.error(err);
            showNotification("error", tA.errorComplete);
          }
        });
      }
    });

    const groupBtn = acceptedSection.querySelector(".tm-openGroupChat");
    groupBtn.addEventListener("click", () => {
      document.querySelectorAll(".tm-acceptedCard")
  ...
  const notifContainer = document.getElementById("tm-appliersNotification");
  notifContainer.innerHTML = "";

  const progressingAppliers = appliers.filter(a => a.applyStatus === "progressing");
  const assignedAppliers = appliers.filter(a => a.applyStatus === "assigned");
  const completedAppliers = appliers.filter(a => a.applyStatus === "completed");

  let notifMessage = "";
  let notifClass = "";
  let newStatus = null;

  if (assignedAppliers.length && !progressingAppliers.length && !completedAppliers.length && taskData.status !== "Assigned") {
    notifMessage = tA.confirmAssign;
    notifClass = "progressing";
    newStatus = "Assigned";
  } else if (progressingAppliers.length && !completedAppliers.length && taskData.status !== "Progressing") {
    notifMessage = tA.confirmProgress;
    notifClass = "progressing";
    newStatus = "Progressing";
  } else if (completedAppliers.length && taskData.status !== "Completed") {
    notifMessage = tA.confirmCompletion;
    notifClass = "completed";
    newStatus = "Completed";
  }

  if (notifMessage && newStatus) {
    const notifHTML = `
      <div class="tm-applierNotif ${notifClass}">
        <span><i class="ri-information-line"></i> ${notifMessage}</span>
        <button class="tm-confirmTask">${tA.confirm}</button>
      </div>
    `;
    notifContainer.innerHTML = notifHTML;

    const confirmBtn = notifContainer.querySelector(".tm-confirmTask");
    confirmBtn.addEventListener("click", async () => {
      try {
        ...
        updateWorkRowMeta(opportunityId, newStatus);
        loadWorkAppliers(opportunityId, { ...taskData, status: newStatus });
        showNotification("success", tA.taskUpdated);
      } catch (err) {
        console.error(err);
        showNotification("error", tA.errorTask);
      }
    });
  }
}

function updateWorkRowMeta(opportunityId, newStatus) {
  const row = document.querySelector(`.tm-taskRow[data-id="${opportunityId}"]`);
  if (!row) return;

  const statusDot = row.querySelector(".tm-statusDot");
  const statusText = row.querySelector(".tm-taskRow-status");

  if (statusDot) {
    statusDot.className = `tm-statusDot ${newStatus}`;
  }
  if (statusText) {
...
function calculateRawScore(cv, task) {
  const match = calculateMatching(cv, task);
  ...
  const totalScore =
    (match.experienceCount || 0) * weights.experience +
    (match.educationCount || 0) * weights.education +
    (match.certificationsCount || 0) * weights.certifications +
    ((match.skillMatchPercent || 0) / 100) * weights.skills +
    ((match.languageMatchPercent || 0) / 100) * weights.languages +
    (match.interestsCount || 0) * weights.interests;

  return totalScore;
}

function buildFullCVHTML(userData, taskData = {}, opportunityId, applicantId, tAct) {
  ...
  const match = calculateMatching(cv, taskData);

  return `
    <div class="tm-fullCV-header">
      <div class="tm-fullCV-actions">
        <button class="tm-cvAction accept" data-task="${opportunityId}" data-user="${applicantId}">
          <i class="ri-check-line"></i> ${tAct.accept}
        </button>

        <button class="tm-cvAction assign" data-task="${opportunityId}" data-user="${applicantId}">
          <i class="ri-hand-coin-line"></i> ${tAct.assign}
        </button>

        <button class="tm-cvAction follow" data-task="${opportunityId}" data-user="${applicantId}">
          <i class="ri-user-follow-line"></i> ${tAct.follow}
        </button>

        <button class="tm-cvAction decline" data-task="${opportunityId}" data-user="${applicantId}">
          <i class="ri-close-line"></i> ${tAct.decline}
        </button>

        <button class="tm-cvAction report" data-task="${opportunityId}" data-user="${applicantId}">
          <i class="ri-flag-line"></i> ${tAct.report}
        </button>
      </div>

      <button class="tm-closeFullCV">&times;</button>
    </div>

    <div class="tm-recoKpiGrid">
      ${renderRecoKPI(tAct.experience, match.experienceCount)}
      ${renderRecoKPI(tAct.education, match.educationCount)}
      ${renderRecoKPI(tAct.certifications, match.certificationsCount)}
      ${renderRecoKPI(tAct.skillsMatch, match.skillMatchPercent + "%", true)}
      ${renderRecoKPI(tAct.languagesMatch, match.languageMatchPercent + "%", true)}
      ${renderRecoKPI(tAct.interests, match.interestsCount)}
    </div>

    <h4 style="margin-top:0; font-size:23px;">
      ${personal.fullName || ""}
    </h4>

    ${renderFullSection(tAct.experienceTitle, experience.map(e =>
      `<div><strong>${e.title}</strong> - ${e.company}<br>
      <small>${e.startDate} → ${e.endDate}</small><br>
      ${e.responsibilities}</div>`
    ))}

    ${renderFullSection(tAct.educationTitle, education.map(e =>
      `<div><strong>${e.degree}</strong> - ${e.institution}<br>
      <small>${e.startDate} → ${e.endDate}</small></div>`
    ))}

    ${renderFullSection(tAct.certificationsTitle, certifications.map(c =>
      `<div>${c.description} - ${c.institution}</div>`
    ))}

    ${renderSkillsSection(tAct.skillsTitle, skills)}
    ${renderSkillsSection(tAct.languagesTitle, languages)}

    ${interests.length ? `
      <div class="tm-fullCV-hobbies">
        <strong>${tAct.interestsTitle}:</strong> ${interests.join(", ")}
      </div>
    ` : ""}
  `;
}

function renderFullSection(title, items) {
...
function renderSkillsSection(title, items) {
  if (!items.length) return "";
  return `
    <div style="margin-top:12px;">
      <strong>${title}</strong>
      ${items.map(renderSavedSkillCircle).join("")}
    </div>
  `;
}

function calculateMatching(cv, task) {

  const taskSkills = task.skills || [];
  const taskLanguages = task.languages || [];
  const taskCategory = task.industry?.toLowerCase() || "";
  const taskDescription = task.description?.toLowerCase() || "";
  const taskType = task.type?.toLowerCase() || "";

  const userSkills = cv.skills || [];
  const userLanguages = cv.languages || [];
  const userExperience = cv.experience || [];
  const userEducation = cv.education || [];
  const userCertifications = cv.certifications || [];
  const userInterests = cv.interests || [];

  /* ===================== SKILLS ===================== */

  const matchedSkills = taskSkills.filter(ts =>
    userSkills.some(us =>
      us.name?.toLowerCase() === ts.name?.toLowerCase()
    )
  );

  /* ===================== LANGUAGES ===================== */

  const matchedLanguages = taskLanguages.filter(tl =>
    userLanguages.some(ul =>
      ul.name?.toLowerCase() === tl.name?.toLowerCase()
    )
  );

  /* ===================== EXPERIENCE MATCH ===================== */

  const matchedExperience = userExperience.filter(exp => {
    const text = `
      ${exp.title || ""} 
      ${exp.company || ""} 
    ...
    return (
      (taskCategory && text.includes(taskCategory)) ||
      (taskType && text.includes(taskType)) ||
  ...
  const matchedEducation = userEducation.filter(edu => {
    const text = `
      ${edu.degree || ""} 
      ${edu.institution || ""}
    `.toLowerCase();

    return taskCategory && text.includes(taskCategory);
  });

  /* ===================== CERTIFICATIONS MATCH ===================== */

  const matchedCertifications = userCertifications.filter(cert => {
    const text = `
      ${cert.description || ""} 
      ${cert.institution || ""}
    `.toLowerCase();
  ...
  const matchedInterests = userInterests.filter(interest => {
    ...
    return (
      (taskCategory && text.includes(taskCategory)) ||
      (taskDescription && text.includes(text))
    );
  });

  return {
    experienceCount: matchedExperience.length,
    educationCount: matchedEducation.length,
    certificationsCount: matchedCertifications.length,
    interestsCount: matchedInterests.length,

    matchedSkills,
    matchedLanguages,

    skillMatchPercent: taskSkills.length
      ? Math.round((matchedSkills.length / taskSkills.length) * 100)
      : 0,

    languageMatchPercent: taskLanguages.length
      ? Math.round((matchedLanguages.length / taskLanguages.length) * 100)
      : 0
  };
}

function renderRecoKPI(label, value, isPercent = false) {
  ...
  if (isPercent) {
    ...
    if (numeric >= 70) variant = "strong";
    else if (numeric >= 40) variant = "medium";
  } else {
    if (value >= 3) variant = "strong";
    else if (value >= 1) variant = "medium";
  }

  return `
    <div class="tm-recoKpi ${variant}">
...
async function updateApplicantStatus(opportunityId, applicantId, newStatus, extraData = {}) {
  const selectedLang = getCurrentLang();
  const t = cvbAppliersActTranslations[selectedLang] || cvbAppliersActTranslations['en'];
  const m = t.applierStatusMessages;

  try {
    const applyRef = doc(db, "opportunities", opportunityId, "applies", applicantId);
    ...
    let title = m.assignSuccessTitle;
    let message = m.assignSuccessMsg;

    switch (newStatus) {
      case "accepted":
        title = m.acceptSuccessTitle;
        message = m.acceptSuccessMsg;
        break;

      case "assigned":
        title = m.assignSuccessTitle;
        message = m.assignSuccessMsg;
        break;

      case "declined":
        title = m.declineSuccessTitle;
        message = m.declineSuccessMsg;
        break;

      case "saved":
        title = m.followSuccessTitle;
        message = m.followSuccessMsg;
        break;

      case "reported":
        title = m.reportSuccessTitle;
        message = m.reportSuccessMsg;
        break;
    }
    ...
    console.error(err);
    showNotification("error", m.errorTitle, m.errorMsg);
  }
}

function attachFullCVActions(container, tAct, taskData) {

  const handleAction = async (btn, status) => {
    if (!btn) return;

    btn.addEventListener("click", async () => {
      const opportunityId = btn.dataset.task;
      const userId = btn.dataset.user;
      await updateApplicantStatus(opportunityId, userId, status);
      await loadWorkAppliers(opportunityId, taskData);
    });
  };

  handleAction(container.querySelector(".accept"), "accepted");
  handleAction(container.querySelector(".assign"), "assigned");
  handleAction(container.querySelector(".decline"), "declined");
  handleAction(container.querySelector(".follow"), "saved");

  const reportBtn = container.querySelector(".report");
  if (reportBtn) {
    reportBtn.addEventListener("click", () => {

      const kpiGrid = container.querySelector(".tm-recoKpiGrid");

      if (kpiGrid?.previousElementSibling?.classList.contains("tm-reportBox")) return;

      const reportBox = document.createElement("div");
      reportBox.className = "tm-reportBox";
      reportBox.innerHTML = `
        <textarea 
          placeholder="${tAct.reportPlaceholder}"
          class="tm-reportInput"></textarea>

        <div class="tm-reportActions">
          <button class="tm-submitReport">${tAct.submitReport}</button>
          <button class="tm-cancelReport">${tAct.cancel}</button>
        </div>
      `;

      if (kpiGrid) {
        kpiGrid.insertAdjacentElement("beforebegin", reportBox);
      } else {
        container.appendChild(reportBox);
      }

      reportBox.querySelector(".tm-submitReport")
        .addEventListener("click", () => {
          const reason = reportBox
            .querySelector(".tm-reportInput")
            .value.trim();

          if (!reason) return;

          updateApplicantStatus(
            reportBtn.dataset.task,
            reportBtn.dataset.user,
            "reported",
            { reportReason: reason }
          );
        ...
        reportBox.querySelector(".tm-cancelReport")
          .addEventListener("click", () => {
            reportBox.remove();
        });
...
function openGroupChat(opportunityId, teamCount = 0) {
  openChat(opportunityId, "group", teamCount);
}

function openPrivateChat(opportunityId, userId) {
  openChat(opportunityId, "private_" + userId);
}

async function openChat(opportunityId, chatId, teamCount = 0) {
  const selectedLang = getCurrentLang();
  ...
  const container = document.querySelector(".tm-chatContainer");
  if (!container) return;

  const chatBox = document.createElement("div");
  chatBox.className = "tm-chatBox";

  let headerAvatar = "/images/default_avatar.png";
  let headerName = "Conversation";
  let isGroup = chatId === "group";

  if (isGroup) {
    headerAvatar = null;
    headerName = teamCount > 0
      ? `${t.announcements} • ${teamCount} ${t.members}`
      : t.announcements;
  }

  if (!isGroup && chatId.startsWith("private_")) {
    ...
    try {
      ...
      if (profileSnap.exists()) {
        ...
        headerAvatar = profile.avatar || "/images/default_avatar.png";
        headerName = fullName || "User";
      }
    } catch (err) {
      console.error(err);
    }
  ...
  chatBox.innerHTML = `
      ...
      <div class="tm-chatHeader-left">
        ...
        ${
          isGroup
            ...
            : `<img class="tm-chatHeader-avatar" src="${headerAvatar}">`
        }

        <div class="tm-chatHeader-info">
          <div class="tm-chatHeader-name">${headerName}</div>
          <div class="tm-chatHeader-status"></div>
        </div>
      </div>

      <div class="tm-chatHeader-actions">
        <button class="tm-chatAction open">
          <i class="ri-briefcase-line"></i>
        </button>

        <button class="tm-chatAction mute">
          <i class="ri-notification-off-line"></i>
        </button>

        <button class="tm-chatAction stop">
    ...
    <div class="tm-chatMessages">
      ...
      <div class="tm-chatEmpty">
        ...
        <div><strong>${t.emptyTitle}</strong></div>
        <div style="font-size:12px;">${t.emptySubtitle}</div>
      </div>
    </div>
  ...
  const chatDocRef = doc(db, "opportunities", opportunityId, "chats", chatId);
  if (chatStatusUnsubscribe) chatStatusUnsubscribe();
  chatStatusUnsubscribe = onSnapshot(chatDocRef, (snap) => {
    let status = "Awaiting";
    if (snap.exists()) {
    ...
    statusEl.classList.remove("muted", "stopped");
  ...
  const openBtn = chatBox.querySelector(".tm-chatAction.open");
  const muteBtn = chatBox.querySelector(".tm-chatAction.mute");
  const stopBtn = chatBox.querySelector(".tm-chatAction.stop");
  ...
  const messagesCol = collection(db, "opportunities", opportunityId, "chats", chatId, "messages");
  ...
  chatUnsubscribe = onSnapshot(realtimeQuery, async snap => {
    snap.docChanges().forEach(async change => {
      if (change.type === "added") {
        ...
        await appendMessage(messagesContainer, msg, t, false, true);
  ...
  sendBtn.addEventListener("click", async () => {
    if (statusEl.textContent === t.statusStopped) return;
    const text = input.value.trim();
    if (!text) return;
    const chatRef = doc(db, "opportunities", opportunityId, "chats", chatId);
...
async function loadMessages(messagesCol, container, t, prepend = false) {
  ...
  if (snap.empty) {
    ...
    if (!hasMessages && loadMoreWrapper) {
  ...
  let firstNewSeparatorLabel = null;

  for (const msg of messages) {
    const dateObj = msg.createdAt?.toDate?.() || new Date();
    const messageDay = formatChatDate(dateObj, t);

    if (!firstNewSeparatorLabel) {
      firstNewSeparatorLabel = messageDay;
    }
  ...
  if (prepend) {
    ...
    if (firstExistingSeparator && lastNewSeparator) {
      if (firstExistingSeparator.textContent === lastNewSeparator.textContent) {
    ...
    if (loadMoreWrapper) {
      container.insertBefore(fragment, loadMoreWrapper.nextSibling);
    } else {
...
function applyTranslations(lang) {
  ...
  translateWorkHeader(lang);

  if (currentOpenedTaskId) {
    displayJobsInView(currentOpenedTaskId);
    (async () => {
      try {
        const taskSnap = await getDoc(doc(db, "opportunities", currentOpenedTaskId));
        if (taskSnap.exists()) {
          const taskData = taskSnap.data();
          loadWorkAppliers(currentOpenedTaskId, taskData);
        }
      } catch (err) {
        console.error("Reload appliers error:", err);
      }
    })();
  }
  ...
  onAuthStateChanged(auth, (user) => {
    if (user) {
      ...
      updateUserSection(null);
      loadWorkManagerDashboard(null);
      loadUserWorkTable(null);
    }
  });
}
...
const workManagerTranslations = {
  en: {
    headerTitle: "Opportunities Manager",
    headerSubtitle: "Track and manage your tasks, jobs and opportunities.",

    viewTable: "Table",
    viewBoard: "Board",
    viewGantt: "Gantt",

    totalTasks: "Opportunities",
    totalViews: "Total Views",
    totalApplies: "Applications",
    ...
    emptyState: {
      defaultTitle: "Select an opportunity",
      defaultDescription: "Choose an opportunity from the list to view its full details, manage status, and review applicants.",
      noTasksTitle: "No opportunities found",
      noTasksDescription: "You haven't created any opportunities yet. Click the 'Create Opportunity' button to get started.",
      noFilterResultsTitle: "No matching opportunities",
      noFilterResultsDescription: "Try adjusting your search or filter to find what you're looking for."
    }
  },

  fr: {
    headerTitle: "Gestion des opportunités",
    headerSubtitle: "Suivez et gérez vos tâches, missions et opportunités.",

    viewTable: "Tableau",
    viewBoard: "Kanban",
    viewGantt: "Gantt",

    totalTasks: "Opportunités",
    totalViews: "Total des vues",
    totalApplies: "Candidatures",
    ...
    emptyState: {
      defaultTitle: "Sélectionner une opportunité",
      defaultDescription: "Choisissez une opportunité dans la liste pour voir tous ses détails, gérer son statut et consulter les candidats.",
      noTasksTitle: "Aucune opportunité trouvée",
      noTasksDescription: "Vous n'avez pas encore créé d'opportunité. Cliquez sur le bouton 'Créer une opportunité' pour commencer.",
      noFilterResultsTitle: "Aucune opportunité correspondante",
      noFilterResultsDescription: "Essayez d'ajuster votre recherche ou vos filtres pour trouver ce que vous cherchez."
    }
  },

  ar: {
    headerTitle: "إدارة الفرص",
    headerSubtitle: "تتبع وأدر مهامك، وظائفك وفرصك.",
    ...
    emptyState: {
      defaultTitle: "اختر فرصة",
      defaultDescription: "اختر فرصة من القائمة لعرض تفاصيلها الكاملة، وإدارة حالتها، ومراجعة المتقدمين.",
      noTasksTitle: "لم يتم العثور على فرص",
      noTasksDescription: "لم تقم بإنشاء أي فرص بعد. انقر على زر 'إنشاء فرصة' للبدء.",
      noFilterResultsTitle: "لا توجد فرص مطابقة",
      noFilterResultsDescription: "حاول تعديل بحثك أو الفلتر للعثور على ما تبحث عنه."
    }
  },

  es: {
    headerTitle: "Gestor de oportunidades",
    headerSubtitle: "Realiza un seguimiento y gestiona tus tareas, trabajos y oportunidades.",

    viewTable: "Tabla",
    viewBoard: "Tablero",
    viewGantt: "Gantt",

    totalTasks: "Oportunidades",
    totalViews: "Total de vistas",
    totalApplies: "Solicitudes",
    ...
    emptyState: {
      defaultTitle: "Seleccionar una oportunidad",
      defaultDescription: "Elige una oportunidad de la lista para ver todos sus detalles, gestionar su estado y revisar los solicitantes.",
      noTasksTitle: "No se encontraron oportunidades",
      noTasksDescription: "Aún no has creado ninguna oportunidad. Haz clic en el botón 'Crear oportunidad' para comenzar.",
      noFilterResultsTitle: "No hay oportunidades que coincidan",
      noFilterResultsDescription: "Intenta ajustar tu búsqueda o filtro para encontrar lo que buscas."
    }
  },

  de: {
    headerTitle: "Gelegenheitsverwaltung",
    headerSubtitle: "Verfolgen und verwalten Sie Ihre Aufgaben, Jobs und Gelegenheiten.",

    viewTable: "Tabelle",
    viewBoard: "Board",
    viewGantt: "Gantt",

    totalTasks: "Gelegenheiten",
    totalViews: "Gesamtaufrufe",
    totalApplies: "Bewerbungen",
    ...
    emptyState: {
      defaultTitle: "Gelegenheit auswählen",
      defaultDescription: "Wähle eine Gelegenheit aus der Liste, um alle Details anzuzeigen, den Status zu verwalten und Bewerber zu überprüfen.",
      noTasksTitle: "Keine Gelegenheiten gefunden",
      noTasksDescription: "Du hast noch keine Gelegenheiten erstellt. Klicke auf 'Gelegenheit erstellen', um zu beginnen.",
      noFilterResultsTitle: "Keine passenden Gelegenheiten",
      noFilterResultsDescription: "Passe deine Suche oder Filter an, um das Gesuchte zu finden."
    }
  },

  pt: {
    headerTitle: "Gestor de oportunidades",
    headerSubtitle: "Acompanhe e gerencie suas tarefas, trabalhos e oportunidades.",
    ...
    emptyState: {
      defaultTitle: "Selecionar uma oportunidade",
      defaultDescription: "Escolha uma oportunidade da lista para ver todos os detalhes, gerenciar o status e revisar os candidatos.",
      noTasksTitle: "Nenhuma oportunidade encontrada",
      noTasksDescription: "Você ainda não criou nenhuma oportunidade. Clique no botão 'Criar oportunidade' para começar.",
      noFilterResultsTitle: "Nenhuma oportunidade correspondente",
      noFilterResultsDescription: "Tente ajustar sua pesquisa ou filtro para encontrar o que procura."
    }
  },

  zh: {
    headerTitle: "机会管理",
    headerSubtitle: "跟踪并管理您的任务、工作和机会。",
    ...
    emptyState: {
      defaultTitle: "选择机会",
      defaultDescription: "从列表中选择一个机会以查看其完整详情、管理状态和查看申请人。",
      noTasksTitle: "未找到机会",
      noTasksDescription: "您还没有创建任何机会。点击\"创建机会\"按钮开始。",
      noFilterResultsTitle: "没有匹配的机会",
      noFilterResultsDescription: "尝试调整搜索或筛选条件以找到您想要的内容。"
    }
  },

  ja: {
    headerTitle: "機会管理",
    headerSubtitle: "タスク、仕事、機会を追跡して管理します。",
    ...
    emptyState: {
      defaultTitle: "機会を選択",
      defaultDescription: "リストから機会を選択して、詳細の表示、ステータスの管理、応募者の確認を行います。",
      noTasksTitle: "機会が見つかりません",
      noTasksDescription: "まだ機会を作成していません。『機会を作成』ボタンをクリックして始めましょう。",
      noFilterResultsTitle: "一致する機会がありません",
      noFilterResultsDescription: "検索条件やフィルターを調整してお探しの内容を見つけてください。"
    }
  },

  ru: {
    headerTitle: "Управление возможностями",
    headerSubtitle: "Отслеживайте и управляйте своими задачами, работами и возможностями.",

    viewTable: "Таблица",
    viewBoard: "Доска",
    viewGantt: "Гантт",

    totalTasks: "Возможности",
    ...
    emptyState: {
      defaultTitle: "Выберите возможность",
      defaultDescription: "Выберите возможность из списка, чтобы просмотреть все детали, управлять статусом и просматривать соискателей.",
      noTasksTitle: "Возможности не найдены",
      noTasksDescription: "Вы еще не создали ни одной возможности. Нажмите кнопку 'Создать возможность', чтобы начать.",
      noFilterResultsTitle: "Нет подходящих возможностей",
      noFilterResultsDescription: "Попробуйте изменить поиск или фильтр, чтобы найти то, что ищете."
    }
  }
};
function translateWorkHeader(lang) {
  const t = workManagerTranslations[lang] || workManagerTranslations['en'];

  const header = document.querySelector('.tm-header');
  if (!header) return;

  const title = header.querySelector('.tm-headerLeft h2');
  const subtitle = header.querySelector('.tm-headerLeft p');
  if (title) title.textContent = t.headerTitle;
  if (subtitle) subtitle.textContent = t.headerSubtitle;

  const viewBtns = header.querySelectorAll('.tm-viewBtn');
  viewBtns.forEach(btn => {
    const view = btn.getAttribute('data-view');
    if (view === "table") btn.title = t.viewTable;
    if (view === "board") btn.title = t.viewBoard;
    if (view === "gantt") btn.title = t.viewGantt;
  });

  const kpis = header.querySelectorAll('.tm-kpiCard span');
  if (kpis[0]) kpis[0].innerHTML = `<i class="ri-briefcase-line"></i> ${t.totalTasks}`;
  if (kpis[1]) kpis[1].innerHTML = `<i class="ri-eye-line"></i> ${t.totalViews}`;
  if (kpis[2]) kpis[2].innerHTML = `<i class="ri-send-plane-line"></i> ${t.totalApplies}`;
  if (kpis[3]) kpis[3].innerHTML = `<i class="ri-line-chart-line"></i> ${t.conversionRate}`;
  
  const topBar = document.querySelector('.tm-tasksTopBar');
  if (!topBar) return;

  const searchInput = topBar.querySelector('#tm-searchInput');
  if (searchInput) {
    searchInput.placeholder = t.searchPlaceholder;
  }

  const periodFilter = topBar.querySelector('#tm-periodFilter');
  if (periodFilter) {
    const options = periodFilter.querySelectorAll('option');
    options.forEach(option => {
      const val = option.value;
      if (val === "all") option.textContent = t.periodAll;
      if (val === "7") option.textContent = t.period7;
      if (val === "30") option.textContent = t.period30;
      if (val === "90") option.textContent = t.period90;
      if (val === "365") option.textContent = t.period365;
      if (val === "custom") option.textContent = t.periodCustom;
    });
  }

  const tasksViewContainer = document.getElementById("tm-tasksView");
  if (tasksViewContainer) {
    const emptyStateDiv = tasksViewContainer.querySelector('.tm-emptyState');
    if (emptyStateDiv) {
      const emptyStateType = emptyStateDiv.getAttribute('data-empty-type') || 'default';
      const customMessage = emptyStateDiv.getAttribute('data-custom-message') || '';
      const emptyState = t.emptyState || {
        defaultTitle: "Select a task",
        defaultDescription: "Choose a task from the list to view its full details, manage status, and review appliers.",
        noTasksTitle: "No tasks found",
      ...
      let newTitle = emptyState.defaultTitle;
      let newDescription = emptyState.defaultDescription;

      if (emptyStateType === "noTasks") {
        newTitle = emptyState.noTasksTitle;
        newDescription = emptyState.noTasksDescription;
      } else if (emptyStateType === "noFilterResults") {
        newTitle = emptyState.noFilterResultsTitle;
        newDescription = emptyState.noFilterResultsDescription;
      } else if (emptyStateType === "custom" && customMessage) {
        newTitle = customMessage;
        newDescription = emptyState.defaultDescription;
      }

      const titleElement = emptyStateDiv.querySelector('h3');
      const descriptionElement = emptyStateDiv.querySelector('p');
...
const cvbTaskEmptyTranslations = {
  en: {
    noTasksTitle: "No tasks found",
    noTasksDesc: "Start by posting your first task or learn more about our platform.",
    postOpportunityLink: "Post your first task",
    learnMoreLink: "Learn more",
    postOpportunityHref: "opportunities/create.html",
    learnMoreHref: "/"
  },
  fr: {
    noTasksTitle: "Aucune tâche trouvée",
    noTasksDesc: "Commencez par publier votre première tâche ou en savoir plus sur notre plateforme.",
    postOpportunityLink: "Publier votre première tâche",
  ...
  es: {
    ...
    noTasksDesc: "Comienza publicando tu primera tarea o aprende más sobre nuestra plataforma.",
    postOpportunityLink: "Publica tu primera tarea",
  ...
  de: {
    ...
    noTasksDesc: "Beginnen Sie, indem Sie Ihre erste Aufgabe veröffentlichen oder mehr über unsere Plattform erfahren.",
    postOpportunityLink: "Erste Aufgabe veröffentlichen",
  ...
  ar: {
    ...
    noTasksDesc: "ابدأ بنشر مهمتك الأولى أو تعرف أكثر على منصتنا.",
    postOpportunityLink: "أنشئ مهمتك الأولى",
    learnMoreLink: "تعرف أكثر",
    postOpportunityHref: "opportunities/create.html",
    learnMoreHref: "/"
  },
  pt: {
    noTasksTitle: "Nenhuma tarefa encontrada",
    noTasksDesc: "Comece publicando sua primeira tarefa ou saiba mais sobre nossa plataforma.",
    postOpportunityLink: "Publique sua primeira tarefa",
    learnMoreLink: "Saiba mais",
    postOpportunityHref: "opportunities/create.html",
    learnMoreHref: "/"
  },
  ja: {
    noTasksTitle: "タスクが見つかりません",
    noTasksDesc: "最初のタスクを投稿するか、プラットフォームの詳細をご覧ください。",
    postOpportunityLink: "最初のタスクを投稿する",
    learnMoreLink: "詳細を見る",
    postOpportunityHref: "opportunities/create.html",
    learnMoreHref: "/"
  },
  zh: {
    noTasksTitle: "未找到任务",
    noTasksDesc: "发布您的第一个任务或了解有关我们平台的更多信息。",
    postOpportunityLink: "发布您的第一个任务",
    learnMoreLink: "了解更多",
    postOpportunityHref: "opportunities/create.html",
    learnMoreHref: "/"
  },
  ru: {
    ...
    noTasksDesc: "Начните с публикации вашей первой задачи или узнайте больше о нашей платформе.",
    postOpportunityLink: "Опубликовать первую задачу",
...
const cvbActionsTranslations = {
  en: {
    duplicateSuccessTitle: "Task duplicated",
    duplicateSuccessMsg: "Your task has been successfully duplicated.",
    duplicateErrorTitle: "Duplicate failed",
    duplicateErrorMsg: "Something went wrong while duplicating the task.",

    deleteConfirmTitle: "Delete task",
    deleteConfirmMsg: "Are you sure you want to delete this task?",

    deleteSuccessTitle: "Task deleted",
    deleteSuccessMsg: "The task has been removed successfully.",
    deleteErrorTitle: "Delete failed",
    deleteErrorMsg: "Something went wrong while deleting the task.",
  },

  fr: {
    duplicateSuccessTitle: "Tâche dupliquée",
    duplicateSuccessMsg: "Votre tâche a été dupliquée avec succès.",
    duplicateErrorTitle: "Échec de duplication",
    duplicateErrorMsg: "Une erreur s'est produite lors de la duplication.",

    deleteConfirmTitle: "Supprimer la tâche",
    deleteConfirmMsg: "Êtes-vous sûr de vouloir supprimer cette tâche ?",

    deleteSuccessTitle: "Tâche supprimée",
    deleteSuccessMsg: "La tâche a été supprimée avec succès.",
    deleteErrorTitle: "Échec de suppression",
    deleteErrorMsg: "Une erreur s'est produite lors de la suppression.",
  },

  es: {
    duplicateSuccessTitle: "Tarea duplicada",
    duplicateSuccessMsg: "Tu tarea ha sido duplicada exitosamente.",
    duplicateErrorTitle: "Error al duplicar",
    duplicateErrorMsg: "Algo salió mal al duplicar la tarea.",

    deleteConfirmTitle: "Eliminar tarea",
    deleteConfirmMsg: "¿Estás seguro de que quieres eliminar esta tarea?",

    deleteSuccessTitle: "Tarea eliminada",
    deleteSuccessMsg: "La tarea ha sido eliminada exitosamente.",
    deleteErrorTitle: "Error al eliminar",
    deleteErrorMsg: "Algo salió mal al eliminar la tarea.",
  },

  de: {
    duplicateSuccessTitle: "Aufgabe dupliziert",
    duplicateSuccessMsg: "Ihre Aufgabe wurde erfolgreich dupliziert.",
    duplicateErrorTitle: "Duplizieren fehlgeschlagen",
    duplicateErrorMsg: "Beim Duplizieren der Aufgabe ist ein Fehler aufgetreten.",

    deleteConfirmTitle: "Aufgabe löschen",
    deleteConfirmMsg: "Sind Sie sicher, dass Sie diese Aufgabe löschen möchten?",

    deleteSuccessTitle: "Aufgabe gelöscht",
    deleteSuccessMsg: "Die Aufgabe wurde erfolgreich entfernt.",
    deleteErrorTitle: "Löschen fehlgeschlagen",
    deleteErrorMsg: "Beim Löschen der Aufgabe ist ein Fehler aufgetreten.",
  },

  ar: {
    duplicateSuccessTitle: "تم نسخ المهمة",
    duplicateSuccessMsg: "تم نسخ المهمة بنجاح.",
    duplicateErrorTitle: "فشل النسخ",
    duplicateErrorMsg: "حدث خطأ أثناء نسخ المهمة.",

    deleteConfirmTitle: "حذف المهمة",
    deleteConfirmMsg: "هل أنت متأكد أنك تريد حذف هذه المهمة؟",

    deleteSuccessTitle: "تم حذف المهمة",
    deleteSuccessMsg: "تم حذف المهمة بنجاح.",
    deleteErrorTitle: "فشل الحذف",
    deleteErrorMsg: "حدث خطأ أثناء حذف المهمة.",
  },

  pt: {
    duplicateSuccessTitle: "Tarefa duplicada",
    duplicateSuccessMsg: "Sua tarefa foi duplicada com sucesso.",
    duplicateErrorTitle: "Falha na duplicação",
    duplicateErrorMsg: "Algo deu errado ao duplicar a tarefa.",

    deleteConfirmTitle: "Excluir tarefa",
    deleteConfirmMsg: "Tem certeza de que deseja excluir esta tarefa?",

    deleteSuccessTitle: "Tarefa excluída",
    deleteSuccessMsg: "A tarefa foi removida com sucesso.",
    deleteErrorTitle: "Falha na exclusão",
    deleteErrorMsg: "Algo deu errado ao excluir a tarefa.",
  },

  ja: {
    duplicateSuccessTitle: "タスクを複製しました",
    duplicateSuccessMsg: "タスクは正常に複製されました。",
    duplicateErrorTitle: "複製に失敗しました",
    duplicateErrorMsg: "タスクの複製中にエラーが発生しました。",

    deleteConfirmTitle: "タスクを削除",
    deleteConfirmMsg: "このタスクを削除してもよろしいですか？",

    deleteSuccessTitle: "タスクを削除しました",
    deleteSuccessMsg: "タスクは正常に削除されました。",
    deleteErrorTitle: "削除に失敗しました",
    deleteErrorMsg: "タスクの削除中にエラーが発生しました。",
  },

  zh: {
    duplicateSuccessTitle: "任务已复制",
    duplicateSuccessMsg: "您的任务已成功复制。",
    duplicateErrorTitle: "复制失败",
    duplicateErrorMsg: "复制任务时出错。",

    deleteConfirmTitle: "删除任务",
    deleteConfirmMsg: "您确定要删除此任务吗？",

    deleteSuccessTitle: "任务已删除",
    deleteSuccessMsg: "任务已成功删除。",
    deleteErrorTitle: "删除失败",
    deleteErrorMsg: "删除任务时出错。",
  },

  ru: {
    duplicateSuccessTitle: "Задача дублирована",
    duplicateSuccessMsg: "Ваша задача успешно дублирована.",
    duplicateErrorTitle: "Ошибка дублирования",
    duplicateErrorMsg: "Произошла ошибка при дублировании задачи.",

    deleteConfirmTitle: "Удалить задачу",
    deleteConfirmMsg: "Вы уверены, что хотите удалить эту задачу?",

    deleteSuccessTitle: "Задача удалена",
    deleteSuccessMsg: "Задача успешно удалена.",
    deleteErrorTitle: "Ошибка удаления",
    deleteErrorMsg: "Произошла ошибка при удалении задачи.",
  }
};

const cvbNotifyTranslations = {
  en: {
...
const cvbAppliersTranslations = {
  en: {
    ...
    taskAppliers: "Task Appliers",
    acceptedList: "Accepted List",
    ...
    confirmAssign: "You already assigned the task to some appliers, can you confirm the task assignation?",
    confirmProgress: "You already confirmed progress for some assigned appliers, can you confirm the task progress?",
    confirmCompletion: "You already confirmed that some executors have completed, can you confirm the task completeness?",

    taskUpdated: "Task status updated.",
    taskProgressing: "Task status changed to Progressing.",
    taskCompleted: "Task marked as completed.",

    errorProgress: "Failed to confirm progress.",
    errorComplete: "Failed to confirm completion.",
    errorTask: "Failed to confirm task."
  },
  ...
  fr: {
    ...
    taskAppliers: "Candidats à la tâche",
    acceptedList: "Liste acceptée",
    ...
    confirmAssign: "Vous avez déjà assigné la tâche, confirmer l'assignation ?",
    confirmProgress: "Vous avez confirmé un avancement, confirmer le progrès global ?",
    confirmCompletion: "Vous avez confirmé des complétions, confirmer la tâche ?",

    taskUpdated: "Statut de la tâche mis à jour.",
    taskProgressing: "Statut changé en progression.",
    taskCompleted: "Tâche marquée comme terminée.",

    errorProgress: "Erreur confirmation progression.",
    errorComplete: "Erreur confirmation complétion.",
    errorTask: "Erreur confirmation tâche."
  },
  ...
  ar: {
    ...
    taskAppliers: "المتقدمون للمهمة",
    acceptedList: "القائمة المقبولة",
    ...
    confirmAssign: "لقد قمت بتعيين المهمة، هل تؤكد ذلك؟",
    confirmProgress: "تم تأكيد التقدم، هل تؤكد تقدم المهمة؟",
    confirmCompletion: "تم تأكيد الإكمال، هل تؤكد إتمام المهمة؟",

    taskUpdated: "تم تحديث حالة المهمة",
    taskProgressing: "تم تغيير الحالة إلى قيد التنفيذ",
    taskCompleted: "تم إكمال المهمة",

    errorProgress: "فشل تأكيد التقدم",
    errorComplete: "فشل تأكيد الإكمال",
    errorTask: "فشل تأكيد المهمة"
  },
  ...
  es: {
    ...
    taskAppliers: "Solicitantes de tarea",
    acceptedList: "Lista aceptada",
    ...
    confirmAssign: "Ya asignaste la tarea a algunos solicitantes, ¿puedes confirmar la asignación de la tarea?",
    confirmProgress: "Ya confirmaste el progreso de algunos asignados, ¿puedes confirmar el progreso de la tarea?",
    confirmCompletion: "Ya confirmaste que algunos ejecutores han completado, ¿puedes confirmar la finalización de la tarea?",

    taskUpdated: "Estado de la tarea actualizado.",
    taskProgressing: "Estado de la tarea cambiado a En progreso.",
    taskCompleted: "Tarea marcada como completada.",

    errorProgress: "Error al confirmar el progreso.",
    errorComplete: "Error al confirmar la finalización.",
    errorTask: "Error al confirmar la tarea."
  },
  ...
  de: {
    ...
    taskAppliers: "Aufgabenbewerber",
    acceptedList: "Akzeptierte Liste",
    announcements: "Ankündigungen",
    noName: "Kein Name",

    workStarted: "Arbeit begonnen",
    workCompleted: "Arbeit abgeschlossen",
    confirm: "Bestätigen",

    confirmAssign: "Sie haben die Aufgabe bereits einigen Bewerbern zugewiesen, können Sie die Aufgabenverteilung bestätigen?",
    confirmProgress: "Sie haben bereits den Fortschritt für einige zugewiesene Bewerber bestätigt, können Sie den Aufgabenfortschritt bestätigen?",
    confirmCompletion: "Sie haben bereits bestätigt, dass einige Ausführende abgeschlossen haben, können Sie die Vollständigkeit der Aufgabe bestätigen?",

    taskUpdated: "Aufgabenstatus aktualisiert.",
    taskProgressing: "Aufgabenstatus auf 'In Bearbeitung' geändert.",
    taskCompleted: "Aufgabe als abgeschlossen markiert.",

    errorProgress: "Fehler bei der Bestätigung des Fortschritts.",
    errorComplete: "Fehler bei der Bestätigung des Abschlusses.",
    errorTask: "Fehler bei der Bestätigung der Aufgabe."
  },
  ...
  pt: {
    ...
    taskAppliers: "Candidatos à tarefa",
    acceptedList: "Lista aceita",
    ...
    confirmAssign: "Você já atribuiu a tarefa a alguns candidatos, pode confirmar a atribuição da tarefa?",
    confirmProgress: "Você já confirmou o progresso para alguns atribuídos, pode confirmar o progresso da tarefa?",
    confirmCompletion: "Você já confirmou que alguns executores concluíram, pode confirmar a conclusão da tarefa?",

    taskUpdated: "Status da tarefa atualizado.",
    taskProgressing: "Status da tarefa alterado para Em andamento.",
    taskCompleted: "Tarefa marcada como concluída.",

    errorProgress: "Falha ao confirmar o progresso.",
    errorComplete: "Falha ao confirmar a conclusão.",
    errorTask: "Falha ao confirmar a tarefa."
  },
  ...
  zh: {
    ...
    confirmAssign: "您已将任务分配给一些申请者，确认任务分配吗？",
    confirmProgress: "您已确认一些被分配者的进度，确认任务进度吗？",
    confirmCompletion: "您已确认一些执行者已完成，确认任务完成吗？",

    taskUpdated: "任务状态已更新。",
    taskProgressing: "任务状态已更改为进行中。",
    taskCompleted: "任务已标记为已完成。",

    errorProgress: "确认进度失败。",
    errorComplete: "确认完成失败。",
    errorTask: "确认任务失败。"
  },
  ...
  ja: {
    ...
    confirmAssign: "すでにタスクを一部の応募者に割り当てました。タスクの割り当てを確認しますか？",
    confirmProgress: "すでに一部の割り当てられた応募者の進捗を確認しました。タスクの進捗を確認しますか？",
    confirmCompletion: "すでに一部の実行者が完了したことを確認しました。タスクの完了を確認しますか？",

    taskUpdated: "タスクステータスが更新されました。",
    taskProgressing: "タスクステータスが「進行中」に変更されました。",
    taskCompleted: "タスクが完了としてマークされました。",

    errorProgress: "進捗の確認に失敗しました。",
    errorComplete: "完了の確認に失敗しました。",
    errorTask: "タスクの確認に失敗しました。"
  },
  ...
  ru: {
    ...
    taskAppliers: "Соискатели задачи",
    acceptedList: "Принятый список",
    ...
    confirmAssign: "Вы уже назначили задачу некоторым соискателям, можете подтвердить назначение задачи?",
    confirmProgress: "Вы уже подтвердили прогресс для некоторых назначенных соискателей, можете подтвердить прогресс задачи?",
    confirmCompletion: "Вы уже подтвердили, что некоторые исполнители завершили работу, можете подтвердить выполнение задачи?",

    taskUpdated: "Статус задачи обновлен.",
    taskProgressing: "Статус задачи изменен на 'В процессе'.",
    taskCompleted: "Задача отмечена как завершенная.",

    errorProgress: "Не удалось подтвердить прогресс.",
    errorComplete: "Не удалось подтвердить завершение.",
    errorTask: "Не удалось подтвердить задачу."
  }
};

const cvbAppliersActTranslations = {
  en: {
    // ACTIONS
    accept: "Accept Applicant",
    assign: "Assign Task",
    follow: "Save Applicant",
    decline: "Decline Applicant",
    report: "Report",

    // REPORT BOX
    reportPlaceholder: "Explain why you are reporting this applicant...",
    ...
    languagesMatch: "Languages Match",
    interests: "Interests",
    ...
    applierStatusMessages: {
      acceptSuccessTitle: "Applicant accepted",
      acceptSuccessMsg: "The applicant has been successfully accepted.",

      assignSuccessTitle: "Task assigned",
      assignSuccessMsg: "The task has been assigned to the applicant.",

      followSuccessTitle: "Applicant saved",
      followSuccessMsg: "The applicant has been saved successfully.",

      declineSuccessTitle: "Applicant declined",
      declineSuccessMsg: "The applicant has been declined.",

      reportSuccessTitle: "Report submitted",
      reportSuccessMsg: "Your report has been sent successfully.",
  ...
  fr: {
    ...
    assign: "Attribuer la tâche",
    follow: "Sauvegarder",
    decline: "Refuser",
    report: "Signaler",

    reportPlaceholder: "Expliquez pourquoi vous signalez ce candidat...",
    ...
    languagesMatch: "Correspondance des langues",
    interests: "Intérêts",
    ...
    applierStatusMessages: {
      acceptSuccessTitle: "Candidat accepté",
      acceptSuccessMsg: "Le candidat a été accepté avec succès.",

      assignSuccessTitle: "Tâche attribuée",
      assignSuccessMsg: "La tâche a été attribuée au candidat.",

      followSuccessTitle: "Candidat sauvegardé",
      followSuccessMsg: "Le candidat a été sauvegardé avec succès.",

      declineSuccessTitle: "Candidat refusé",
      declineSuccessMsg: "Le candidat a été refusé.",

      reportSuccessTitle: "Signalement envoyé",
      reportSuccessMsg: "Votre signalement a été envoyé avec succès.",

      errorTitle: "Action échouée",
  ...
  ar: {
    ...
    reportPlaceholder: "اشرح سبب الإبلاغ عن هذا المترشح...",
    submitReport: "إرسال",
    cancel: "إلغاء",
    ...
    applierStatusMessages: {
      acceptSuccessTitle: "تم قبول المترشح",
      acceptSuccessMsg: "تم قبول المترشح بنجاح.",

      assignSuccessTitle: "تم تعيين المهمة",
      assignSuccessMsg: "تم تعيين المهمة للمترشح.",

      followSuccessTitle: "تم حفظ المترشح",
      followSuccessMsg: "تم حفظ المترشح بنجاح.",

      declineSuccessTitle: "تم رفض المترشح",
      declineSuccessMsg: "تم رفض المترشح.",

      reportSuccessTitle: "تم إرسال التبليغ",
      reportSuccessMsg: "تم إرسال تبليغك بنجاح.",

      errorTitle: "فشل الإجراء",
      errorMsg: "حدث خطأ ما. يرجى المحاولة مرة أخرى."
    }
  },

  es: {
    accept: "Aceptar solicitante",
    assign: "Asignar tarea",
    follow: "Guardar solicitante",
    decline: "Rechazar solicitante",
    report: "Reportar",

    reportPlaceholder: "Explica por qué estás reportando a este solicitante...",
    ...
    languagesMatch: "Coincidencia de idiomas",
    interests: "Intereses",
    ...
    applierStatusMessages: {
      acceptSuccessTitle: "Solicitante aceptado",
      acceptSuccessMsg: "El solicitante ha sido aceptado exitosamente.",

      assignSuccessTitle: "Tarea asignada",
      assignSuccessMsg: "La tarea ha sido asignada al solicitante.",

      followSuccessTitle: "Solicitante guardado",
      followSuccessMsg: "El solicitante ha sido guardado exitosamente.",

      declineSuccessTitle: "Solicitante rechazado",
      declineSuccessMsg: "El solicitante ha sido rechazado.",

      reportSuccessTitle: "Reporte enviado",
      reportSuccessMsg: "Tu reporte ha sido enviado exitosamente.",

      errorTitle: "Acción fallida",
      errorMsg: "Algo salió mal. Por favor, inténtalo de nuevo."
    }
  },

  de: {
    accept: "Bewerber annehmen",
    assign: "Aufgabe zuweisen",
    follow: "Bewerber speichern",
    decline: "Bewerber ablehnen",
    report: "Melden",

    reportPlaceholder: "Erklären Sie, warum Sie diesen Bewerber melden...",
    ...
    languagesMatch: "Sprach-Übereinstimmung",
    ...
    applierStatusMessages: {
      acceptSuccessTitle: "Bewerber angenommen",
      acceptSuccessMsg: "Der Bewerber wurde erfolgreich angenommen.",

      assignSuccessTitle: "Aufgabe zugewiesen",
      assignSuccessMsg: "Die Aufgabe wurde dem Bewerber zugewiesen.",

      followSuccessTitle: "Bewerber gespeichert",
      followSuccessMsg: "Der Bewerber wurde erfolgreich gespeichert.",

      declineSuccessTitle: "Bewerber abgelehnt",
      declineSuccessMsg: "Der Bewerber wurde abgelehnt.",

      reportSuccessTitle: "Meldung gesendet",
      reportSuccessMsg: "Ihre Meldung wurde erfolgreich gesendet.",
  ...
  pt: {
    accept: "Aceitar candidato",
    assign: "Atribuir tarefa",
    follow: "Salvar candidato",
    decline: "Recusar candidato",
    report: "Denunciar",

    reportPlaceholder: "Explique por que você está denunciando este candidato...",
    ...
    languagesMatch: "Compatibilidade de idiomas",
    ...
    applierStatusMessages: {
      acceptSuccessTitle: "Candidato aceito",
      acceptSuccessMsg: "O candidato foi aceito com sucesso.",

      assignSuccessTitle: "Tarefa atribuída",
      assignSuccessMsg: "A tarefa foi atribuída ao candidato.",

      followSuccessTitle: "Candidato salvo",
      followSuccessMsg: "O candidato foi salvo com sucesso.",

      declineSuccessTitle: "Candidato recusado",
      declineSuccessMsg: "O candidato foi recusado.",

      reportSuccessTitle: "Denúncia enviada",
      reportSuccessMsg: "Sua denúncia foi enviada com sucesso.",

      errorTitle: "Ação falhou",
  ...
  zh: {
    ...
    reportPlaceholder: "说明您举报此申请人的原因...",
    submitReport: "提交举报",
    cancel: "取消",
    ...
    applierStatusMessages: {
      acceptSuccessTitle: "申请人已接受",
      acceptSuccessMsg: "申请人已成功接受。",

      assignSuccessTitle: "任务已分配",
      assignSuccessMsg: "任务已分配给申请人。",

      followSuccessTitle: "申请人已保存",
      followSuccessMsg: "申请人已成功保存。",

      declineSuccessTitle: "申请人已拒绝",
      declineSuccessMsg: "申请人已被拒绝。",

      reportSuccessTitle: "举报已提交",
      reportSuccessMsg: "您的举报已成功发送。",
  ...
  ja: {
    ...
    reportPlaceholder: "この応募者を報告する理由を説明してください...",
    submitReport: "報告を送信",
    cancel: "キャンセル",
    ...
    applierStatusMessages: {
      acceptSuccessTitle: "応募者を承認しました",
      acceptSuccessMsg: "応募者は正常に承認されました。",

      assignSuccessTitle: "タスクを割り当てました",
      assignSuccessMsg: "タスクが応募者に割り当てられました。",

      followSuccessTitle: "応募者を保存しました",
      followSuccessMsg: "応募者は正常に保存されました。",

      declineSuccessTitle: "応募者を拒否しました",
      declineSuccessMsg: "応募者は拒否されました。",

      reportSuccessTitle: "報告を送信しました",
      reportSuccessMsg: "あなたの報告は正常に送信されました。",

      errorTitle: "アクションに失敗しました",
      errorMsg: "問題が発生しました。もう一度お試しください。"
    }
  },

  ru: {
    accept: "Принять заявителя",
    assign: "Назначить задачу",
    follow: "Сохранить заявителя",
    decline: "Отклонить заявителя",
    report: "Пожаловаться",

    reportPlaceholder: "Объясните, почему вы жалуетесь на этого заявителя...",
    ...
    languagesMatch: "Соответствие языков",
    interests: "Интересы",
    ...
    applierStatusMessages: {
      acceptSuccessTitle: "Заявитель принят",
      acceptSuccessMsg: "Заявитель успешно принят.",

      assignSuccessTitle: "Задача назначена",
      assignSuccessMsg: "Задача назначена заявителю.",

      followSuccessTitle: "Заявитель сохранен",
      followSuccessMsg: "Заявитель успешно сохранен.",

      declineSuccessTitle: "Заявитель отклонен",
      declineSuccessMsg: "Заявитель отклонен.",

      reportSuccessTitle: "Жалоба отправлена",
      reportSuccessMsg: "Ваша жалоба успешно отправлена.",

      errorTitle: "Ошибка действия",
